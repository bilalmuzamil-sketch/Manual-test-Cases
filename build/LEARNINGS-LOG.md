...
**L0042 (2026-09-10) — A separated-steps case can STILL have its main preconds/expected field broken by an API write; the separated-steps render check will not catch it.** When correcting C43850 (Vladimir's, separated-steps, `custom_steps_separated` with `markdown_editor_id=1`), the 6 steps rendered fine but the **`custom_preconds`** field — a *main markdown field*, written via API — landed in an escaping `<div class="markdown">` container and showed the tester literal `<p>`/`</p>`. Separated-step content (med=1) is loaded client-side and does NOT use the `markdown fr-view` container, so a scan that only looks at `custom_steps_separated` (or the static `<div class="markdown">` fetch, which is empty for separated-steps cases) reports "clean" while the preconds is broken. **Two facts to carry:** (1) the served page for a separated-steps case must be read in a **real browser** (Playwright), not urllib — the steps are AJAX-rendered and absent from the static HTML; scan `document.body.innerText` for literal `<p>|</p>|<br>|<ol>|<li>|<hr>`. (2) Any MAIN field (preconds/steps/expected) you write via API on a case whose container is escaping still needs the **UI/Froala repair** (`inst.html.set` on `#<field>_display`, set the backing `#<field>` value + dispatch `change` to enable `#accept`, Save) to flip it to `fr-view` — even when the rest of the case uses separated steps. The UI save preserved the separated steps untouched (verified: still 6 steps, med=1, content identical). Reinforces L0028 (API write on an already-escaping container re-escapes) and L0034 (check `custom_steps_separated`, not just `custom_steps`).

---

**L0043 (2026-09-10) — SOURCE VERIFICATION READS THE WHOLE DOCUMENT AND EVERY LINK IT NAMES — NEVER JUST THE REQUIREMENT BODY (QA lead, 2026-09-10; promoted to Standing Rule 108).** On the Invoice Design Selection re-verification the QA lead had to prompt three times: (1) read the Questions/answers at the bottom of the spec; (2) open the tickets the spec links (e.g. SV-9872); (3) "read EVERYTHING… open every ticket link ALWAYS… including the links to other sources given in the document." The requirement body (Story 1–5) had NOT changed between reads — every material movement lived in **Section 8.1/8.2 decision cells** (Q6 recorded option (a); Q10 decided; Q12 gained option C; Q17 refined), in the **User Feedback Summary (Section 9)** strings, in the **Change Log**, and in the **linked tickets** (Q7 defect cluster SV-9819/9778/9790/9693 that Legacy reintroduces; SV-9821 now OBSOLETE → replaced by the live SV-9832 Bug still in Code Review; SV-9870 the 45%-paper set). A verification that read only the requirement section would have missed all of it. **THE RULE:** a full source verification reads the ENTIRE source — requirement body + the full question/answer/decision log (Q1..QN, N grows over time, today Q22) + the User Feedback Summary + the Change Log + and it OPENS AND READS every ticket / page / design link the document references, recursively for the ones that bear on the requirement. Decision cells, answered questions, feedback strings and linked tickets are all sources (Rule 57). Encoded in `build/skills/02-SOURCE-CHECK.md` and Standing Rule 108.

---

**L0044 (2026-09-24) — DEFECT EVIDENCE: GO INSIDE TO PROVE · NEVER TRUNCATE THE ANNOTATION · SIZE THE IMAGE TO RENDER LARGE INLINE (QA lead, 2026-09-24).** Three standing rules for defect screenshots, learned filing the Global-Search "Contact match" defects (SV-10439 vendor / SV-10440 customer). **(1) GO INSIDE TO PROVE.** A claim of absence or presence ("there is no such contact", "no contact name contains the term") is proven by *navigating INTO the record and showing the truth on its own page* — open the vendor/customer's **Contacts** tab and show the empty list, or the contacts whose names do not match — never inferred from the search result row or the API response alone. The search row is the *symptom*; the record's own page is the *proof*. (Reinforces skills 19/20 — the verdict comes from the screen.) On a QA branch a hard nav to a detail route can bounce to `/login` under the synthetic token; reach the detail page the way a user does — open the record **from the search modal (client-side Select/Enter)**, then click its Contacts tab. **(2) NEVER TRUNCATE THE ANNOTATION.** Caption/annotation text baked into an evidence image must **wrap to the image width** — never run off the right edge. Measure text width against the frame and wrap; if it still overflows, widen the frame or move the detail into the ticket body. A cut-off caption is a defective deliverable. **The TITLE/header line is an annotation too — wrap it, not just the body captions** (a title that reads fine in the source string still overruns a fixed-width frame; this is exactly how the first cut-off slipped through, 2026-09-24). **VERIFY THE FINISHED IMAGE BEFORE SHIPPING:** assert programmatically that every drawn line's pixel width ≤ `frame − 2·margin`, *and* re-open the final PNG and read it. Shipping an unverified image, or trusting the source string instead of the rendered pixels, is the mistake — and *also re-check which attachment the ticket actually references*, since a re-upload can land under an auto-generated name and leave the description pointing at the old one. **(3) SIZE THE IMAGE TO RENDER LARGE INLINE (Jira).** Jira renders an attached image referenced with *no* width as a tiny (~300px) click-to-open thumbnail, and a full-page ~1680px screenshot referenced at `|width=1000` as a small, unreadable ~600px block. The readable recipe: **CROP** the screenshot to the meaningful region (the modal / the panel / the message — drop the left nav and empty space), size the file to **~950–1000px wide** (native == display, crisp), and reference it **WITH an explicit width** (`!name|width=980!` in a comment, `|width=1100` in a description) so it fills the column instead of thumbnailing. Verified 2026-09-23/24 on SV-10322 and SV-10439/10440. Encoded in `build/skills/06-DEFECT-PREP.md`.
