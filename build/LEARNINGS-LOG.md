...
**L0044 (2026-09-16) — THE "AUTH TO ORDER" PART STATUS HAS TWO PRECONDITIONS; A PART SEEN WITHOUT THEM SHOWS "AWAITING", AND CALLING "AUTH TO ORDER" ABSENT FROM THAT IS A FALSE NEGATIVE (QA lead caught it live).** Build-verifying the Inline free-typed-part cases C45013 (Tech View) / C45054 (Full View) on staging `v26.36.7`, I added free-typed parts and saw their status render as **"Awaiting"** (action **"Receive"**), in BOTH tech and full view, and was about to report that the build does not show the documented **"Auth to order"** status — a build-vs-document deviation. It was WRONG. On a work-order **Parts** row the status **"Auth to order"** (action **"Order"**) appears ONLY when BOTH conditions hold: **(1) the parent LINE is Authorized/Approved** (open Edit Line → its **Status** field reads **"Authorized"**; the line chip shows "Approved"), **AND (2) the part's Source = Vendor** (a special order bought from a vendor), **not Source = Inventory**. A part sourced from Inventory, or a part on a line that is not yet authorized, does **not** show "Auth to order". Once a vendor part is ordered it moves on to **"Awaiting" → Receive**, so "Awaiting" is a LATER state than "Auth to order", not a rename of it. Proof, same build: WO `S2-32218` carried both at once — `(SP-NEW-989)` a vendor part on an Approved line at **"Auth to order" / Order**, and `(SP-NEW-898)` already-ordered at **"Awaiting" / Receive**. The QA lead's screenshot showed the free-typed part `(fdgfdgdfg) fdgdf` with **Source = Vendor** on an **Authorized** line reading **"Auth To Order"**.
