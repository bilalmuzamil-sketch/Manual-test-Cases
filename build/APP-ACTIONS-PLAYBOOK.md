...
- **Payment apply** (for "settles to $0.00" / "paid invoice refuses edit"): `POST /api/parts-catalogue/vendor/payment/create {account_id,payment_amount,payment_method,payment_date,transactions:[…]}` — the `transactions` allocation + `payment_method` enum shape was NOT resolved for SV-9096 (`{id,amount}` gave "Payment amount must be greater than $0.00"); **capture the real payload from the New-Payment UI next time** rather than guessing.
