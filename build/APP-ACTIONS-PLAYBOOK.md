...
## AE. QuickBooks Unexported report + deposits/receives (proven 2026-09-10, SV-9866)
- **Report:** route `/reports/unexported-items`; endpoint `GET /api/bookkeeping/unexported-items?filters[0][field]=groupName&filters[0][value]=customer|vendor|journal_entry`. Row fields `type · typeLabel · group · number · customer · referenceId · errorMessage · date`. Tabs are Customers / Vendors / Journal Entries.
- **⚠️ The table is a Quasar VIRTUAL-SCROLL container, NOT paginated.** 30 rows render, the window does not scroll, and there is **no pagination control** — it looks like "only 30 of 588 are reachable" and that is FALSE. Scroll the `.q-virtual-scroll` ancestor of the `<table>` (32 → 74 rows and onward). Never report a pagination defect here without doing that first (Rule 75).
- **`/administration/QuickBooks` renders BLANK unless `localStorage.bookkeeping_enabled === "true"`.** The route guard reads that localStorage key directly (`isQuickBooksEnabled()`); the value ships in the login payload at `data.details.bookkeeping_enabled`. **Seed it in any boot script** next to `user` / `fe_permissions_wrapper` / `token`, or the page is empty and looks like a dead route. (The sidebar item under Administration → INTEGRATIONS is separately absent on both staging and the QA branches — only IBS shows — so the page cannot be reached by clicking either.)
- **`/api/bookkeeping/settings` is PUT-only** — `GET` returns 405 `Method Not Allowed (Allow: PUT)`. The bookkeeping mappings can be WRITTEN but not READ, so on a shared env you cannot snapshot them to restore afterwards. Do not clear a mapping on staging for a test.
- **Create a deposit:** `POST /api/deposits {workOrderId, amount, paymentMethod:'CASH'|…, depositDate, memo}` → 201 `{creditNumber:"DEP-####", status:"held"}`. UI: Finance tab → `button_new_deposit` → `select_create_deposit_payment_method` / `input_create_deposit_amount` / `input_create_deposit_memo` → **Submit Deposit must be clicked by coordinate** (a global `hasText` locator misses it). Add Deposit is offered on Estimate / Approved / In Progress / Review — **not** on Complete or Invoiced.
- **Customer payment:** `POST /api/customer-account/create-customer-payment` → `{bookkeepingSyncDeferred:true}` when a QuickBooks sync is queued. UI: `button_new_payment` → `select_payment_method` → `button_make_payment`.
- **Receive a vendor part — the payload the screen actually sends:** `POST /api/orders/receive-requested-parts` → **200**, body `{vendor_id, invoice_number, invoice_date, note, total, tax, items:[{id, cost, description, line_id, …}]}`. Blind-shaping this 500s; drive `/order/{orderId}?receive=1&returnTo=WorkOrder&returnId={woId}` and capture it. **Two traps:** (1) the per-item checkbox `checkbox_item_<itemId>` **STARTS TICKED** — clicking it turns receiving OFF and the button greys out with "Select at least one part to receive"; set `input_qty_<itemId>` instead. (2) **Assigning a vendor re-renders the group**, so the `input_invoice_*` / `input_qty_*` ids must be re-enumerated afterwards or `page.fill` times out.
- **`POST /api/inventory/orders/receive-view {orderIds:[…]}`** returns the receive screen's model, including `vendorMissing` per vendor group — handy for checking state without the UI.
- **Invoicing can be gated by IBS:** Create Invoice fires `GET /api/invoices/ibs/retrieveIBSApproval?workOrderId=…` → **400** while the work order shows **"Over Limit"**, and the click then only re-renders the estimate preview (no dialog, no write) — it looks like a dead button. Also seen: `GET /api/invoices/{woId}/details` → **500** on a particular work order, which kills the flow before it starts. Screen candidates by calling `details` first.
- **Work-order list:** `GET /api/work-orders?...&filters[0][field]=status&filters[0][value]=estimate|approved|in_progress|ready_for_review|complete|invoiced|paid` → `data.work_orders` (**not** `data.collection`); `totalPrice` is in **cents**. Statuses: `GET /api/work-orders/statuses`.
