...
checks=[
 ...
 ("Third merge path - changing the VIN from inside a work order - also carries the imported history over with the destination VIN (source's work orders moved too, source removed).","PASSED"),
 ...
 ("Multi-customer safety: with an asset genuinely shared by two customers, merging as one customer moves only THAT customer's imported rows (with the destination VIN); the other customer's history stays put and visible, nothing leaks onto the merged asset, and the shared source asset is NOT deleted (it stays with the other customer).","PASSED"),
]
 ...
 *media(B+"/02-source-history-and-regular-wos.png","Top: the source asset (partial VIN GM055903) carried the 3 imported invoices before the merge - the history the customer said disappeared. Bottom: after the merge the source's regular work orders (S9289-17580/17581) also moved to the merged asset, now under the destination VIN (regression check)."),
 ...
 p(strong("Edit-VIN path: "),t("Editing an asset's VIN to a destination VIN that already exists triggers the merge and carries the imported history over with the destination VIN, source deleted (POST /api/vehicles/change returns 201).")),
 p(strong("Multi-customer scoping: "),t("Set up a genuinely shared asset (same VIN created under both customers = one asset with two owners), with imported rows for C1 (2) and C2 (1). Merging as C1 moved only C1's 2 rows onto the destination (with its VIN); C2's row stayed under C2, still visible; the source asset's ownership dropped C1 but kept C2, so the shared source asset was NOT deleted - it remains available to C2. Nothing from C2 appeared on the destination.")),
 p(strong("Change-VIN-from-a-work-order path: "),t("On a source that had imported history plus two regular work orders, changing the VIN to the destination (POST /api/vehicles/change, VehicleManager::mergeVehicles) returned 201: the imported invoice carried over to the destination with the destination VIN, both regular work orders moved to the destination, and the source was removed. The ticket's known out-of-scope caveat (this path possibly stranding the source's OTHER regular work orders - the 'Walter' note) did NOT reproduce in this run - both work orders moved cleanly.")),
]}
