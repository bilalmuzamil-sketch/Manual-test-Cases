...
- **C53537** (Credit Invoice against several invoices): provenance had cited withdrawn S2-R5/S2-E4 →
  ...
  gave the exact tester flow (no hardcoded ids). **Preconditions:** customer has two unpaid invoices;
  customer has a credit larger than the sum of those two. **Steps (11):** go to the customer → Invoices
  → check the credit (note its number, it is reused) → check the two invoices one by one → New Payment
  → in the New Customer Payment modal uncheck the credit then check it again → New Payment → wait; the
  processed invoices disappear → the remaining credit shows a print icon → download the credit invoice.
  **No CM1-105 / S1-5 / S1-6 literals.** Verified `fr-view` / 0 literal tags / marker last /
