...
const save=()=>fs.writeFileSync(`${DIR}/PR57.json`, JSON.stringify(R,null,1));
...
const mine=[];
// take work orders that are already COMPLETE for this customer -- Approved ones need a foreman
// review before they can be invoiced, which this account cannot grant itself.
for(const w of wos.filter(w=>/^complete$/i.test(String(w.status||'')))){
...
L('this customer\'s COMPLETE work orders: %s', JSON.stringify(mine));
...
await page.screenshot({path:`${EV}/PR57-credit-dialog.png`, fullPage:true});
...
await page.screenshot({path:`${EV}/PR57-after-credit.png`, fullPage:true});
...
await page.screenshot({path:`${EV}/PR57-pay-dialog.png`, fullPage:true});
...
await page.screenshot({path:`${EV}/PR57-after-pay.png`, fullPage:true});
await openInvoices();
rows=await scan();
...
for(const want of ['legacy','modern']){
  ...
  fs.writeFileSync(`${EV}/PR57-${want}-credit.pdf`, Buffer.from(g.b64,'base64'));
