...
## Posted to Jira — comment 75305 (QA lead authorised, 2026-08-20)

`https://shopview.atlassian.net/browse/SV-8815` · comment id **75305**, created
2026-08-20T05:33:12-0500. Source of the exact body: `evidence/adf-followup.json`, generated by
`evidence/build_followup.py` (which lifts nothing by hand — every figure comes from the run).

**Shape: a compact follow-up, not a rebuild of comment 75278.** The standing preference is one
complete comment rather than a chain, and the intention here was a full rebuild — but comment 75278
is **correct**: nothing in it needed changing, only adding. Retyping its ~58 KB of ADF through the
comment tool (whose body is a literal string parameter, and which I could only view truncated) would
have risked corrupting an accurate record to gain tidiness. So the new comment is **self-contained** —
it repeats the verdict, names its own build, and can be read alone — and 75278 stays as the detail.

**Contents:** first line `OVERALL QA STATUS: PASSED` in a success panel · the vendor-vs-customer
credit distinction · checks **36, 37, 38** all PASSED · the two annotated exhibits (22, 23) ·
technical section with the `calculate-tax` figures and its **mixed dollar/cent units**, the
`credit-memos` payload shape, the `list-credit-available-parts` 500 flagged as unrelated to this
change, the named test data, the screen-vs-API split, and the build marker.

**Pre-flight checks, all passed:** both exhibit URLs returned **200** (`HEAD` 404s — the repo's
default branch is a different one, so the branch name must be in the raw URL) · tone gate found **no
self-revision language** and no barred words · build marker re-read immediately before posting and
`index.html` was **byte-identical (sha256)** to the read at the start of the run.

**Read-back:** the API returned the **stored** comment converted to markdown — not an echo of the
JSON, so it proves the ADF parsed and stored. Verified in it: first line correct, **2** media in the
right order (Invoice-total then the line-by-line control) with the right raw URLs, table of header +
**3** rows numbered 36–38, and all four technical paragraphs present.
