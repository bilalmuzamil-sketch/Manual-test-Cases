...
| 11 | **Ruthless Usefulness Audit** | Scores 100% of a suite's cases KEEP / MERGE / WEAK-KEEP / CUT (would a tester ever catch a real, non-duplicated bug with this case?), hunts the 7 named slop patterns, credits the load-bearing coverage, and answers "is the 70%-useless critic right?" honestly. **MANDATORY final gate of every authoring pass (Standing Rule 28)**; also on demand for any existing suite; recommendations only — TestRail merges/cuts need explicit approval (Rule 6). | *"usefulness audit"* / *"slop check"* / *"audit the cases for waste"* / runs automatically after every authoring pass | `build/<project>/quality-audit-<date>/`: `USEFULNESS-AUDIT-<date>.md` (per-area verdict table + headline current→recommended + honest critic paragraph + layman exec paragraph) + `per-case-verdicts.csv` (C-id + link per Rule 8) + `MERGE-PLAN.md` (approvable wholesale or per-group) | `build/RUTHLESS-USEFULNESS-AUDIT-PROCESS.md` — canonical example `build/report-suite/quality-audit-2026-07-28/` |
