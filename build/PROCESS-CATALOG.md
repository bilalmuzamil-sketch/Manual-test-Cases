...
| 13 | **Test-Case Creation (dedicated skill)** | The authoring lane, end to end: the required input set (spec/PRD + epic & stories + Claude design/Figma/technical design + tech plan + PO name) and the Rule-1 refusal to start on a half-spec; the Rule-31 all-sources currency pre-flight; expected behaviour from documents only, sources (a)-(g), never the build (Rule 57); plain layman + build-accurate wording with "VIU-confirm" flags (Rules 7/9); per-story `refs` as `<TICKET> (<spec-anchor>, spec v<N> <date>)` (Rules 20/42); title <=80 chars; API content in an 'API'-titled section (Rule 4); one line per step with `<br>` line breaks, never bare `\n`; the two-sentence Rule-54 provenance line; the AUTOMATION marker last (4 permitted forms incl. the Rule-69 not-available-on-the-build form) with Rule-61 symptom + three outcomes; Rule-43 per-requirement coverage verdicts + Rule-40 surface matrix + Rule-45 outside-in hunt; Rule-46 deliberate-decisions register; Rule-28 Ruthless Usefulness Audit as the mandatory closing gate; new-project onboarding convention; import mirrors its peers 1:1 (Rule 16). Nothing pushed without permission (Rule 6). | *"author the test cases for [project]"* / *"create the cases from this spec"* / *"write the suite for the new project"* / *"cover this new story"* | `build/<project>/` (PROJECT-STATE.md, requirements.md, cases/, testrail-id-map.csv, coverage-matrix.md) + the dated pass folder (FINDINGS / SOURCE-CURRENCY / DELIBERATE-DECISIONS / audit output) + `testrail-import/<project>-v1-testrail-import.csv`/`.xlsx` | `build/skills/10-TEST-CASE-CREATION.md` |
| 14 | **Build Verification (dedicated skill)** | The verification lane: drive existing cases live and produce an observed PASS / DEVIATION / HOLD verdict for each with evidence. Covers the Rule-80 last-done-date-then-ASK gate and the Rule-81 do-not-auto-run source-verification gate (state the last source-verify date + version, ask WITH or WITHOUT, WAIT); build-marker capture at both ends proven byte-identical; Rules 12/13 observed-never-inferred; Rule 14 seed-don't-block; Rule 74 multi-login standard (reset role to template -> assign to Technician quick-login -> test -> restore); Rule 49 provisional findings + RECHECK-QUEUE; Rule 77 validity window (<=3 builds / <=3 source versions, date shown); Rule 60 layer split; Rule 61 the automated suite as monitor and ticket status never as evidence; Rule 57 the build gives only labels + verdict; Rule 25 verbatim deviation citations; Rule 71 automated cases read-assess -> report -> HOLD; Rule 50 exhaustive-then-exact byte verification; Rules 34/47 union-only run sync. Jira creation asked for, never assumed (Rule 62); API-only findings asked separately (Rule 51). | *"verify the cases against the build for [project]"* / *"drive the suite live"* / *"which cases fail on the current build?"* / *"give the testers the defect list"* | `build/<project>/build-verify-<date>/<Project>_Defects-for-Testers_<date>.xlsx` (tab per verdict + Summary, C-ID + link columns, a plain "What needs to be done" on every non-passed row) + FINDINGS / RECHECK-QUEUE / SOURCE-CURRENCY / testrail-execution-log / API-ASK / evidence | `build/skills/11-BUILD-VERIFICATION.md` |
| 15 | **VIU (dedicated skill)** | The VIU lane — what *"VIU the test cases"* means (Rule 10): run `build/BUILD-ACCURATE-WORDING-VIU-PROCESS.md` end to end — capture the exact on-screen labels LIVE, rewrite title/preconds/steps/expected into build-accurate layman wording, live-verify behaviour with evidence, checkpoint-commit, push via `update_case` with a per-case audit log, STAMP/REFRESH the Rule-54 provenance line in the same push, regenerate deliverables, and ALWAYS state the TestRail update status. Adds the Rule-11 always-ask gate, the Rules 80/81 gates, Rules 57/58 (VIU corrects LABELS, never the expectation; an ambiguous source is never resolved from the build - hold and ask, quote-back test), Rule 41 touch-a-case-re-verify-the-whole-case, Rule 50 byte verification with **all text fields sent on every payload** (TestRail re-renders an omitted field), the `<br>`-not-`\n` rendering rule, Rule 9 flag-don't-invent, Rule 22 ask for the live check + access up front, and Rules 26/26a reset roles to template first with persistent re-reset on drift. | *"VIU the test cases for [project]"* / *"do the VIU"* / *"build-accurate wording pass"* | `build/<project>/full-viu-<date>/` (FINDINGS / CHANGES-MADE / testrail-execution-log / RECHECK-QUEUE / SOURCE-CURRENCY / DELIBERATE-DECISIONS / API-ASK / RESUME / evidence) + corrected cases live in TestRail + regenerated import, Blockers Tracker and results workbook | `build/skills/12-VIU.md` |
| 16 | **Handoff — Test-Case Creation session** | The copy-paste briefing that boots a fresh session into the authoring lane: mission + what it must never do, the read-first order (with the warning NOT to read CLAUDE.md whole), the governing rule list with one-line glosses, the ask-first gates, the five new projects with their DEVELOPERS named and the PO explicitly UNKNOWN, the per-project missing-input request list, the definition of done, and the report-back format. | *"give me the handoff for the test-case creation session"* / *"boot a creation session"* | `build/handoffs/HANDOFF-1-TEST-CASE-CREATION.md` (paste it into the new session) | `build/handoffs/HANDOFF-1-TEST-CASE-CREATION.md` |
| 17 | **Handoff — Build-Verification session** | The copy-paste briefing for the verification lane: mission + lane boundaries, read-first order, the rule list, the Rules 80/81 ask-first gates, the five new projects (developers named, PO UNKNOWN), the missing-input list (including fresh `.qa.shopview.com` cookies and a second non-admin sign-in), the definition of done, and the report-back format. | *"give me the handoff for the build-verification session"* / *"boot a verification session"* | `build/handoffs/HANDOFF-2-BUILD-VERIFICATION.md` (paste it into the new session) | `build/handoffs/HANDOFF-2-BUILD-VERIFICATION.md` |
| 18 | **Handoff — VIU session** | The copy-paste briefing for the VIU lane: mission + the hard boundary that VIU never changes what a case EXPECTS, read-first order (the wording+VIU process first), the rule list, the ask-first gates including explicit push authorisation, the five new projects (developers named, PO UNKNOWN) with the Global Search revival caveat (86 cases authored, never pushed, so there is nothing to `update_case` yet), the missing-input list, the definition of done incl. the arithmetic gate, and the report-back format. | *"give me the handoff for the VIU session"* / *"boot a VIU session"* | `build/handoffs/HANDOFF-3-VIU.md` (paste it into the new session); index of all three = `build/handoffs/README.md` | `build/handoffs/HANDOFF-3-VIU.md` |
| 19 | **Test Execution (dedicated skill)** | Execute the EXISTING cases against a running build and record honest results. Picks the run inside Rule-47 scope (and records who owns it), pins the build marker at pass START and pass END and proves it did not move, executes **batched** (Rule 88 — never one tool call per case; the ampersand paging trap of core §3.3 returns an empty set that reads like "no tests"), captures per-case evidence to the defect standard, and applies the **honest-status rule: a case that was not actually executed is NEVER marked Passed** (Rule 12). **Blocked — never skip, never guess** — but decomposed per Rule 68 with its residual stated; a data-state or a login is never a blocker (Rules 14/74). Retest loop doubles as gate check A1 (reproduced twice). Run sync is **union-only with its own per-pass permission** (Rule 34 + core §4.1), and **results are held LOCALLY by default** — never a result into another tester's run; Passed-only even with permission. Cross-references `03-RUN-CHECK` rather than duplicating it. | *"execute the cases for [project]"* / *"run the suite against the build"* / *"record the results"* | `build/<project>/execution-<date>/` (EXECUTION-LOG / RESULTS / BLOCKED / DEFERRED-RUN / RECHECK-QUEUE / evidence) | `build/skills/09-TEST-EXECUTION.md` |
| 20 | **Test Execution & Defect Reporting lane (router)** | The fourth lane, created 2026-08-26. A **thin router** with no procedure of its own: `00` → `09` → `03` → `06` → `04` §6.1 → `13` → `14`, plus the ⛔ SCOPE GATE and the 📖 READING RULE. Its gates: 6, 11, 24, 25, 49/60, 51, 52, 53, 62, 71, 80, 81, 83, 88, 90, 91 and **94**. **The lane executes cases and prepares defects; it does NOT author cases and does NOT run VIU wording passes**, and cross-lane findings route back to the main session (Rule 83). | *"boot the defect lane"* / *"which skill covers executing and reporting defects?"* | (router — points at the files above) | `build/skills/16-TEST-EXECUTION-AND-DEFECTS.md` |
| 21 | **Handoff — Test Execution & Defect Reporting session** | The copy-paste briefing for the fourth lane: the SCOPE GATE and READING RULE, the QA lead's verbatim "irrelevant / obsolete" refusal quote, the **ADMISSIBILITY GATE (Rule 94)**, the ask-first gates, the definition of done, and the report-back format. **Its FIRST task, before any testing, is the REFUSAL POST-MORTEM** — find our refused defect tickets, **READ THE ACTUAL REFUSAL COMMENTS IN JIRA (do not guess them; an unreadable one is NOT VERIFIED)**, record per ticket the stated reason quoted verbatim, the true root cause and the admissibility check that would have caught it, then **PROPOSE** any further gate checks (Rules 72/93 — propose, never self-record). | *"give me the handoff for the defect lane"* / *"boot a test-execution session"* | `build/handoffs/HANDOFF-4-TEST-EXECUTION-AND-DEFECTS.md` + `build/defect-lane/REFUSAL-POSTMORTEM-<date>.md`; index of all four = `build/handoffs/README.md` | `build/handoffs/HANDOFF-4-TEST-EXECUTION-AND-DEFECTS.md` |
| 22 | **Regression Impact, V1 → V2 (dedicated skill)** | **The answer to the QA lead's 2026-08-21 problem, verbatim:** *"I have no way to know what should not be changed in V1 due to V2 specially when V2 is not asking to change it."* A V2 spec describes **only what CHANGES** and is **SILENT about everything else**, and nothing in our process converted that silence into tests — so a V2 build could break a V1 behaviour with every case still passing. **FOUR STEPS:** (1) build the **V1 BEHAVIOUR BASELINE**; (2) map the **V2 DELTA** against it (CHANGED / REMOVED / REPLACED / **SILENT**, with the verbatim V2 quote wherever V2 speaks); (3) derive the **INVARIANT SET = V1 baseline − (changed ∪ removed ∪ replaced)** — **silence defaults to "must not change"**; (4) **ESCALATE THE DANGEROUS SILENCES** (HIGH collateral risk) as PO questions rather than assuming either way. **SOURCE STANDING is the heart of it: DOCUMENTS establish INTENT** (V1 spec + version, epic stories + acceptance criteria, PO answers, designs — authoritative, Rule 57) · **OUR OWN REPO is a FIRST-CLASS V1 SOURCE** (existing V1 cases and their bodies, `requirements.md`, spec exports, PO answer files, design reviews, `PROJECT-STATE.md`; a case DERIVED from V1 documents carries provenance and counts as a documented invariant, a case with missing/vague provenance is a CANDIDATE only (Rule 64), and **our cases are what we TESTED, not everything V1 DOES** — coverage gaps are invisible invariants) · **PRODUCT SOURCE CODE establishes FACT** (the app repo's current release/develop branch — composables, components, handlers plus existing E2E tests; excellent for exhaustively enumerating current behaviour far beyond our coverage and legitimate for a REGRESSION baseline, **but NEVER a source of expectation (Rule 57): a bug in the code would otherwise become an invariant we actively protect**) · **THE CROSS-CHECK RULE** — code agrees with the documents ⇒ STRONG INVARIANT, write the regression case; code CONTRADICTS them ⇒ **a FINDING and a PO DECISION ITEM, never a silent invariant**, and the QA lead's ruling applies verbatim (*"the PO can decide which behaviour to keep"*) · **COLLATERAL RISK** — code/the developer also answer which components, APIs, data models, permission checks and shared pipelines V2 touches and what else depends on them, which is what tells you **WHICH silences are dangerous** · **PRODUCTION OBSERVATION** = candidate invariants needing PO confirmation, never expectations. Also **RETIRES the superseded V1 cases** (worked example: global search must no longer include page search, so asserting it would raise a defect against intended behaviour and be refused as obsolete — Rule 94). **Needs NO build and NO app cookies** — document + case + code analysis. States its **honest limit** plainly. **Standing Rule 96.** | *"this is V2 of [feature]"* / *"what could V2 break?"* / *"regression impact for [project]"* / *"what must not change?"* / **auto-triggered by intake project type (ii)** | `build/<project>/regression-impact-<date>/`: `REGRESSION-IMPACT-MATRIX.md` (one row per V1 behaviour, 7 columns, tally + arithmetic check + honest limit) · `PO-DECISION-REGISTER.md` (6 fields per row: what the system DOES today, what the V1 document SAYS verbatim, what V2 says or that it is SILENT, the options, our recommendation, DECISION + DATE) · `RETIRE-OR-REWRITE-LIST.md` · the regression case set (each marked regression, each tracing to its V1 source) · PO question sheet rows · an OUTSTANDING section | `build/skills/17-REGRESSION-IMPACT-V1-TO-V2.md` |

## The SKILLS — the same work, packaged to be run COLD by a session with no memory

> **`build/skills/` (added 2026-08-13) is the seven-job packaging of everything above**, written so a
> **fresh session with NO memory of this workspace** can pick up one job and do it correctly.
> **Origin, USER DIRECTIVE (2026-08-12), verbatim:** *"In future we have to convert this whole session
> into multiple Skills, one skill per session, so I want you to manage this session accordingly"* — and
> on why: ***"I do not want our hard work to be lost and things start to bite me and cost me my job due
> to this."***
>
> **These do NOT replace the processes above.** The processes are the authority on method and are
> cross-referenced from each skill; the skills add **the cold-start framing, the failures behind each
> rule, and one shared core so a fix lands once instead of seven times**. Where the two ever differ,
> **`CLAUDE.md` and the process docs win** and the skill file is the one to correct.
>
> **🔴 EVERY SKILL BEGINS WITH `build/skills/00-COMMON-CORE.md`** — honesty bar · TestRail write
> discipline + hazards · runs · foreign cases · access mechanics · environment · session survival ·
> git on a shared branch · secrets (**the repo is PUBLIC**) · authority (**the Jira creation hold is
> ACTIVE**) · reader-facing standards · the OUTSTANDING section.
>
> **"VIU" = SOURCE-CHECK + RUN-CHECK + build-accurate wording.** The pass/fail verdict has belonged to
> the **manual tester** since 2026-08-11 (*"you are RIGHT"*), so **"VIU complete" is never said** — the
> accurate phrase is *"source-verified and build-accurate in its preconditions, steps, navigation and
> labels, with the behaviour verdict belonging to the tester."*

| # | Skill | What it does | How to call it (trigger) | Deliverable it produces | Doc |
|---|---|---|---|---|---|
| S0 | **COMMON CORE** | **Read by every skill first.** The cross-cutting discipline in one place — observed-never-inferred; exhaustive-then-exact; two numbers (ours/live); **all three text fields on every `update_case`**; the byte-check that passes when the *payload* is wrong; `updated_on` lying in both directions; `custom_atmstatus: 1` never `3`; `refs` 248-char pattern limit; `get_sections` paging; `case_title`/`case_refs` echoes; union-only run sync; foreign cases hands-off + tell-Vlad; the five false-dead-session traps; seed-don't-block with roles/staff/settings carved out; the seven survival requirements; path-scoped commits + explicit-SHA push; **the public repo and the 12 leaked JWTs**; the creation hold; a blocker blocks only what it blocks. | *(never called alone — every skill opens with it)* | *(no deliverable of its own)* | `build/skills/00-COMMON-CORE.md` |
| S1 | **CASE-BUILD** | Turn the documents into test cases and **prove nothing was missed**: complete inputs first, coverage **re-derived in both directions** (never patched), the **surface matrix**, no case without a source (with the three opposite remedies for "no source"), the **outside-in gap hunt**, the **deliberate-decisions register**, the **Ruthless Usefulness Audit as the closing gate**, then push + **run sync**. | *"Run **CASE-BUILD** for [project]"* / *"author the cases"* / *"did we miss any cases?"* | `SOURCE-CURRENCY.md` · `COVERAGE-MATRIX.md` (one verdict row per requirement, both texts quoted) · `NEW-CASES.md` · `DELIBERATE-DECISIONS.md` (six fields) · `OUTSIDE-IN-GAP-HUNT.md` · `quality-audit/` · oplog · `RESUME.md` | `build/skills/01-CASE-BUILD.md` |
| S2 | **SOURCE-CHECK** | Prove we hold **today's** spec, epic, designs, tech plan and PO answers. The **three staleness traps** — the in-body version, the epic's admin-only timestamp, and **trap (c): a page being new says nothing about a RULE's age** (the input to latest-wins, once applied backwards). Two-tier epic check; **re-read the sources again at write time**; a **deleted requirement** may mean a deleted case, with the Automated flag checked and **Vlad told**. | *"Run **SOURCE-CHECK** for [project]"* / *"is our spec current?"* / *"what changed in the spec?"* / **implicitly at the start of every other skill** | `SOURCE-CURRENCY.md` (per source: identifier · version · date checked · CURRENT/STALE/PARTIAL, with a PARTIAL naming its exact shortfall) + `SPEC-DIFF-<date>.md` with the per-requirement verdict table | `build/skills/02-SOURCE-CHECK.md` |
| S3 | **RUN-CHECK** | Open the product and prove **a tester could actually execute each case** — the five runnability checks. **The build is the CHECK, never the AUTHOR**; **cosmetic vs substantive** with the recognition test and the new hiding place it created; **probes that cannot fail** (40+ false absences in two days, not one a product fault); **our own instrumentation creating the evidence**; `textContent` **vs** computed style, both readings needed; visible text vs accessible name; **FE-blocks/BE-allows is a PASS**; roles reset first and destructive edits **scheduled last**; a bug-fix deploy does **not** stale a pass. | *"Run **RUN-CHECK** for [project]"* / *"walk the steps"* / *"are these cases runnable?"* / *"the branch has been rebuilt"* | **`DIVERGENCES.md` — written EVEN WHEN EMPTY** · `RUNNABILITY.md` · `FINDINGS.md` (build marker on every verdict + the honest driven-this-pass split) · `LABEL-DIFF.md` · `CHANGES-MADE.md` · execution log · redacted `evidence/` · `RECHECK-QUEUE.md` | `build/skills/03-RUN-CHECK.md` |
| S4 | **TESTER-READY** | Make the suite fit to hand over: the **cold read** for coherence, the **five cross-case contradiction sweeps**, the cross-read that finds **cases already Passed while held** (19 of 91, 16 saying Passed — with our own share owned), the **skip list** with *"mark Blocked, don't Pass"* alongside the three-outcome expect-fail instruction, the **run sync**, and the **generated** brief. | *"Run **TESTER-READY** for [project]"* / *"the testers start tomorrow"* / *"what should they run and what should they skip?"* | `TESTER-BRIEF-<date>.md` (generated, never transcribed) + `HOW-THE-NUMBERS-WERE-DERIVED.md` (marker census, the arithmetic gate shown **both ways**, foreign cases named, set-equality proof) | `build/skills/04-TESTER-READY.md` |
| S5 | **PROJECT-REPORT** | The per-project completion **TABLE, delivered before the next project starts**. Seven columns, with **build-verified and steps-walked reported SEPARATELY** (76 vs 28 of 176 — merging them would have overstated Schedule threefold the day before a release), **never "VIU complete"**, **every figure derived live with the read time stamped**, and **what is left ITEMISED** with each item traced to who it waits on and **every blocker proved first**. | *"Run **PROJECT-REPORT** for [project]"* / *"where are we on [project]?"* / *"what is left?"* / **automatically before starting the next project** | The table in chat + `build/<project>/READINESS-<date>.md` (previous one kept and marked SUPERSEDED) + its working file + the register updated the same turn | `build/skills/05-PROJECT-REPORT.md` |
| S6 | **DEFECT-PREP** | Build a defect that **cannot be challenged — then stop at the button**. The **eight-item evidence bar** (source quoted with version and date; annotated screenshots; **exact named test data** + what was ruled out; build marker; **duplicate search with the JQL recorded**; the required shape; a **written pre-filing self-challenge**; and the **Rule-24 / closed-ticket / status-as-proxy / API-only** nonsense checks), the reachability test asked separately every time, the `Story Defect` shape, the seven-section format and the inline-image mechanism. **NOTHING IS FILED — the creation hold is ACTIVE.** | *"Run **DEFECT-PREP** for [finding]"* / *"is this filable?"* / *"prepare the ticket"* | `build/<project>/defect-pack-<date>/`: `TICKET-<n>.md` · `SELF-CHALLENGE.md` · `DUPLICATE-SEARCH.md` · `API-SPLIT.md` · `CASE-IMPACT.md` (kept OUT of the ticket) · redacted `evidence/` · `NOT-FILED.md` | `build/skills/06-DEFECT-PREP.md` |
| S7 | **PO-QUESTIONS** | **One sheet, in plain words, SENT LAST.** Every row names its **project and feature** because one PO owns several; extremely simplified with A/B options; the mapping on a **QA-only tab**; **ask again whenever an answer is unclear**; **product decisions only, never a defect**; and **every question proved necessary first**. **Counter-limit: a genuine blocker is raised IMMEDIATELY and never deferred onto the sheet.** | *"Run **PO-QUESTIONS** for [project]"* / *"what do we need from Branko / Chris / Milos?"* / *"is the sheet ready to send?"* | `<Project>_Questions-for-<PO Name>_<date>.xlsx` + `.md` (2 tabs: reader-facing questions · QA-only mapping), **shipped attached to the S5 report that says everything else is done** | `build/skills/07-PO-QUESTIONS.md` |
| S8 | **RECOVER** | **Establish what a killed pass actually landed — BY CONTENT — and finish it.** Fetch and fast-forward FIRST (a clean tree is not a current one — one checkout was **110 commits behind** while reporting *clean*, and a recovery pass wrongly concluded six passes' work was lost); read the dead oplog as a **hypothesis**, where an **intent with no outcome is the point it died**; **sweep `/tmp` before it dies and commit what is there**; verify every claimed op against live field by field into **LANDED / NOT LANDED / LANDED BUT WRONG / deliberately dropped**; prove the runs untouched on the **graded fields only**; and **REBUILD a staged plan, never replay it.** | *"Run **RECOVER** for [pass]"* / *"the last pass was killed"* / *"did we lose anything?"* / *"the container restarted"* | `STATE.md` (one row per claimed op, with C-id + how it was verified) · `VERDICT.md` (**in two halves — what was not lost, AND what was**) · committed `evidence/` · `RESUME.md` with a **rebuild** recipe | `build/skills/08-RECOVER.md` |

**Index + how they compose: `build/skills/README.md`.**
**Chain:** CASE-BUILD → SOURCE-CHECK → RUN-CHECK → TESTER-READY, with PROJECT-REPORT gating the move to
the next project, DEFECT-PREP raised by RUN-CHECK, and PO-QUESTIONS sent with the report.

## Notes on choosing between the close ones
- **Skills vs processes:** the **processes** (#1–#12) are the authority on *method* and stay canonical;
  the **skills** (S0–S8) are the cold-start packaging of the same work, and each cross-references the
  process it draws on. **Reach for a skill when a fresh session must pick the job up unattended;
  reach for a process when you need the full method.** S1 composes #2/#11/#12; S3 composes #1's live
  half; S4 composes #11's cold read; S7 is Rules 55/66's sheet; **S8 is Standing Rule 29's recovery half**.
