...
| 13 | **Source Snapshot & Change-Only Delta Tracking** | The runner's **steady-state tracking loop** for a project once its cases exist. Takes ONE committed snapshot of every source's **currency identifier** (Confluence version #, epic child-set + story statuses + changelog high-water, Figma node hashes / Claude-design state, tech-plan version, PO answer dates, build marker — never surface dates, Rule 31 traps). On every later run: re-pull all sources LIVE → **DIFF against the snapshot** → map each change to the affected cases (internal ID + C-id + link) via the id-map `refs` + coverage matrix → emit a **CHANGE-ONLY report** (`build/<slug>/CHANGES-<date>.md`, one row per change with **verbatim** old→new + the cases it hits + the action for the other session) → **refresh the snapshot**. If nothing moved, says exactly that. **This session REPORTS the delta only; the OTHER session corrects the cases from that report (Rule 20) — no test-case edits on a tracking run.** | *"track [project]"* / *"check [project] for changes"* / *"what changed on [project]"* / *"snapshot the sources for [project]"* | `build/<slug>/SOURCE-SNAPSHOT.json` (refreshed each run) + `build/<slug>/CHANGES-<date>.md` (the delta — the ONLY deliverable of a tracking run) | `build/SOURCE-SNAPSHOT-CHANGE-TRACKING-PROCESS.md` |
