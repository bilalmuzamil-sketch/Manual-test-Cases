 ...
 *media(B+"/03-edge-multi-invoice-remove-one.png","Edge case (row 8) - a batch with several invoices. A Sent batch of two invoices ($2,384.17 = Lansing $1,113.83 + Dorr $1,270.34). Removing the Dorr invoice leaves Lansing in the batch and the batch total recomputes to $1,113.83."),
 *media(B+"/04-edge-rebatch.png","Edge case (row 7) - re-batching. The Dorr invoice (which had been moved back to Ready To Send) is ticked and Create Batch is chosen; it appears again as a Sent batch ($1,270.34). An invoice that was removed can be batched again normally."),
 *media(B+"/05-edge-remove-last.png","Edge case (row 6) - removing the last invoice. Sent shows 5 batches including a single-invoice Dorr batch ($1,270.34); after removing that one invoice the batch is emptied and disappears from Sent (4 batches left), and the invoice returns to Ready To Send."),
 rule(),
 h("How to reproduce"),
 ...
 p(strong("Balance / edge cases (all shown in the evidence above): "),t("Removing the Dorr invoice ($1,270.34) from a two-invoice Sent batch ($2,384.17) recomputed the batch to $1,113.83 and left the other invoice (Lansing) in place. Removing the last invoice in a batch emptied it, the batch dropped out of Sent, and the invoice returned to Ready To Send. A returned invoice re-batches normally (Ready To Send -> Create Batch -> shows under Sent). A credit line has no Remove button; calling remove on a credit returns 400 (not found) - only invoices are removable. This directly supports the ticket's Impact notes (being able to issue a credit against, or re-handle, an invoice that was batched in error).")),
