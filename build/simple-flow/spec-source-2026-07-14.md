<!--
PROVENANCE
Source file : 533141e9-SimpleMode_StreamlinedWorkOrderCompletionBulkReceiving_3.doc
Upload dir  : /root/.claude/uploads/dd1d42ba-2c47-5229-9b17-b8f94e3eb99a/
Uploaded    : 2026-07-14 (Confluence "Exported From Confluence" MHTML export)
Format      : Confluence "Save as .doc" = MHTML (multipart/related; one text/html
              part, quoted-printable)
Parsed via  : Python email (MIME) -> html.parser text extractor -> plain text
              (/tmp/mhtml_extract.py). Same MIME+HTMLParser method as the prior
              snapshots; minor spacing/emphasis (**/~~) artifacts are cosmetic.
Parsed on   : 2026-07-14 (Simple Flow project ONLY; PO = Milos; Epic SV-7301)
Doc version : Version STRING still "Draft for build — V2.4" (UNCHANGED), BUT a NEW
              change-log row dated 2026-07-14 (@Milos Vasic) was added -> content is
              de-facto V2.5. DO NOT trust the version string.
Verdict     : NEWER than the 2026-07-13 (_2) upload. Genuine NEW deltas beyond the
              4 prior UNAPPLIED deltas (Δ1-Δ4):
              Δ5 = NEW Story 16 R12 (auto-complete trigger: last open line resolved
                   -> review OFF auto-Completes WO / review ON -> Ready for Review;
                   applies to single/bulk/split/delete-line) + R13 (clock-out
                   exception -> Ready for Review even when review OFF) + new AC lines
                   + change-log row. Resolves the SF-SET-10 / Jira SV-8303 signal.
              Δ6 = NEW S1-R9 clause: settings ALSO apply if a completed WO is
                   REOPENED (still never retroactive to already-completed WOs).
              Δ7 = Story 10 S10-R2 (PN-add -> first-class inventory/catalog part)
                   STRUCK THROUGH (deprecated/removed as a requirement) - reverses
                   requirements.md V2.4 note #6. Confirm with Milos (last-update-wins).
              Δ1-Δ4 all STILL PRESENT and unchanged. See spec-diff-2026-07-14.md.
Note        : READABLE SNAPSHOT for traceability only. Authoritative authoring
              baseline remains requirements.md. The ~~strikethrough~~ / **bold**
              markers below are literal artifacts of the source's rich-text markup.
-->

# Simple Mode — Streamlined WO Completion & Bulk Receiving — SOURCE SNAPSHOT (2026-07-14, doc _3)
...
Simple Flow Design (Claude Design) · QA env — sv7301 (POC)
...
Draft for build — V2.4 (line approval = all must be approved; sell-price mandatory at save + orderable-from-line; editable cost on Accept-Delivery; core resolution in Stories 3/4/8/10/16; in sync with Jira SV-7696…SV-7710 + SV-7870 + SV-7876 + SV-8183)
...
S1-R9 — Save settings. Persists all of the above org-wide, to
future completions only
(never retroactively). This Settings will be applied if someone reopen the already completed WO
...
Gated by (existing atom)

 |

See/edit the WO Settings page
...
Settings › App Settings (settingsApp) — inherits the existing settings route guard; no new gating

 |

Run completion — change WO status
...
Approve all lines (hard gate to complete)
...
WO Lines: Create & Edit + Full View (Tech View hides Approve) → BE ROLE_WORK_ORDER::VIEW + CREATE_AND_EDIT

 |

Enter mileage / VIN / engine hours in completion modal
...
Resolve inventory / special-order cores (Ok / Not OK)
...
Add a vendorless / no-part-number part (manual sell)
...
WO Lines: Create & Edit + See Financial Data (seeFinancialData)

 |

Pick inventory parts in completion modal (auto-pick off)
...
Pick Parts (woPickParts)

 |

Background order + create POs on completion
...
Order Parts (woOrderParts) → requires See Financial Data

 |

Receive on the WO (line Receive button, completion "Receive parts")
...
FE: Order Parts. BE (ReceiveRequestedParts): OR of ROLE_DELIVERY_CREATE_AND_EDIT, ROLE_WORK_ORDER_PART_CREATE, ROLE_WORK_ORDER_CREATE_AND_EDIT

 |

Bulk Receive page (accountant, PO-list driven)
...
Vendor & Order Mgmt: Create & Edit (hasPartsPermissions) + See Financial Data

 |

Assign vendor to a vendor-missing PO / merge / keep-separate
...
Vendor & Order Mgmt: Create & Edit

 |

Inline part-number fix → first-class inventory/catalog part
...
Catalog & Inventory: Create & Edit

 |

Cost/sell fields on receive screens (field locking)
...
See Financial Data; sell auto-locks once WO invoiced/paid (state gate)

 |

Mark Reviewed / sign-off; VIN captured by reviewer
...
Review Work Orders (woReviewWorkOrders) + reviewer ≠ completer (NET-NEW hard rule); VIN entry → WO Lines: Create & Edit

 |

Waiting-on-Parts column (visibility)
...
Work Orders: View; click-through suppressed without the receive gate
...
Invoicing & Payments: Create & Edit + See Financial Data
...
Mixed line approval added to Stories 3/4/16 (auto-approve off → only approved lines complete; unapproved stay open, non-blocking). Core parts marked deferred (placeholder) pending a designed solution.
...
Core-parts resolution folded in across Stories 3/4/8/10/16 — inventory cores gated in the completion modal (after Pick); special-order cores at receive (required) or the Create Invoice gate (optional, receive-then-resolve). No core-engine change. §8 core question marked resolved.
