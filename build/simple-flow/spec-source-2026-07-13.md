<!--
PROVENANCE
Source file : 1a94a925-SimpleMode_StreamlinedWorkOrderCompletionBulkReceiving_2.doc
Upload dir  : /root/.claude/uploads/dd1d42ba-2c47-5229-9b17-b8f94e3eb99a/
Uploaded    : 2026-07-13 (Confluence "Exported From Confluence" MHTML export)
...
Parsed on   : 2026-07-13 (Simple Flow project ONLY; PO = Milos; Epic SV-7301)
Doc version : "Draft for build — V2.4" (version STRING unchanged; change-log last
              row still 2026-07-08).
Verdict     : BYTE-IDENTICAL CONTENT RE-DELIVERY of the prior 2026-07-10 upload
              (b91efa39-...BulkReceiving_1.doc). Extracted text md5 is identical
              (234420851652275204d3748d8a353612); only the outer MHTML export
              wrapper (file md5) differs (a re-export). The 4 prior UNAPPLIED
              deltas (Δ1 VIN drop from Story-4 modal; Δ2 Story-4 unapproved-line =
              Complete disabled+tooltip; Δ3 new receive gates S13-R6/R7;
              Δ4 Mark-Reviewed optional-note removed) are ALL present and UNCHANGED.
              NO new deltas. See spec-diff-2026-07-13.md.
Note        : This is a READABLE SNAPSHOT for traceability only. Authoritative
              extract for authoring remains requirements.md. Minor spacing/bold
              artifacts from the HTML parse are cosmetic. Content is line-for-line
              the same as spec-source-2026-07-10.md.
-->

# Simple Mode — Streamlined WO Completion & Bulk Receiving — SOURCE SNAPSHOT (2026-07-13)
...
On → vendor parts generate POs + the receiving step (today's behavior). Off → no POs created for vendor parts. Default ON.
...
S10-R2 — When a part number is added, the part becomes a first-class inventory/catalog part (POs-on receive): an existing number links the line to the existing item (updates stock + received cost + Part History, without overwriting the catalog description/category); a new number creates a new item.
