...
## 0-A. BUILD-ACCURATE WORDING + VIU PASS — IN PROGRESS (2026-07-13, QA-lead authorized)

Combined build-accurate-wording + VIU pass running area by area
...
screenshots → `screenshots/wording-2026-07-13/`. **Checkpoint = committed
`cases/*.json` `fresh_run:2026-07-13` + the audit log.** Env baseline captured +
restored each run (`/tmp/simple-flow/settings-baseline-0713.json`); Tech never
role-swapped; roles matrix fresh + no drift (`roles-matrix-2026-07-13.md`).

**Areas TESTER-READY (done + pushed to TestRail, all 200/OK, 0 errors):**
- **SF-SET (15)** — 12 Verified / 1 Deviation (SF-SET-03 Create-POs toggle absent) /
  1 Blocked-Env (SF-SET-08 brand-new org non-seedable) / 1 VIU-Pending (SF-SET-10).
  13 updated + 2 no-op.
- **SF-COMP (23)** — 19 Verified / 3 VIU-Pending (delta cases SF-COMP-16/21/22 —
  need a settings-managed drive) / 1 Blocked-Env (SF-COMP-06 No-PO path, toggle
  absent). 5 updated + 18 no-op. Completed disposable WOs S2-15795, S2-15825 during
  surface capture (labor-only, harmless).
- **SF-PERM (10)** — all 10 Verified (re-checked vs the fresh roles matrix + settings
  BE atom 403/200). 9 updated + 1 no-op.

**Remaining areas (not yet processed this pass):** SF-REV, SF-VAL, SF-WOP, SF-UX,
SF-VEND, SF-RCV, SF-BULK, SF-PNFIX, SF-VPART, SF-VMIS, SF-POSEL, SF-INV, SF-PART(n/a),
SF-CORE, SF-TECH, SF-QB (do QB LAST — Blocked-Env, needs a human in QuickBooks).
Still-pending delta re-VIU on a confirmed-idle env: SF-COMP-16/21/22 plus the receive
cluster deltas (SF-VAL-02/06/11, SF-VEND-04/06, SF-RCV-06, SF-PNFIX-05). Milos
Round-3 still pending. **SF-QB-09 is unmapped in `testrail-id-map.csv`** (follow-up:
add its C# before pushing SF-QB). Final deliverables (import CSV, Blockers Tracker,
Results workbook) to be regenerated at the END of the full pass.

---

## 0. CURRENT STATE AT A GLANCE (read this first)

**Tally (2026-07-13 — after V2.4 Δ1-Δ4 applied; authoritative, regenerated by
`build_results_workbook.py` / `gen_blockers.py`):**

| VIU status field (`cases/*.json`) | Count |    | Blocker category (tracker) | Count | Owner |
|---|---:|---|---|---:|---|
| VIU-Verified | **114** |    | READY (VIU-Verified, uploadable now) | **111** | — |
| VIU-Pending | **34** |    | BLOCKED — VIU PENDING (QA) | **39** | QA |
| Open-Question | **5** |    | BLOCKED — MILOS ANSWER | **13** | Milos (PO) |
| Pending / Retest (V2.4 Δ 2026-07-13) | **10** |    | BLOCKED — BUG/RULING | **0** | — |
|  |  |    | BLOCKED — DEV NOT BUILT | **0** | Dev (Stories 7/8/9/14 built) |
| **TOTAL** | **163** |    | **TOTAL** | **163** | |

(Total rose 162 → 163 with the new case SF-VEND-06. The 10 Pending/Retest cases are
the V2.4 Δ1-Δ4 cases whose expected changed 2026-07-13 — they classify under
BLOCKED — VIU PENDING (QA) until the live re-VIU. The field count VIU-Pending 34 vs
blocker VIU-PENDING(QA) 39 differ because the Pending/Retest cases + a few
MILOS-owned VIU-Pending cases shift buckets.)

**✅ DONE 2026-07-13 — 4 V2.4 spec deltas APPLIED to cases + pushed to TestRail**
...
- Every case whose expected changed now carries `viu_status: "Pending / Retest —
  expected changed by V2.4 Δ (2026-07-13), needs live re-VIU"` + `fresh_run:
  2026-07-13`. **NEXT TASK = the wording + VIU pass** to re-verify these live on
  sv7301 (Story-4 modal field set, disabled+tooltip affordance, S13-R6/R7 receive
  blocks, Mark-Reviewed note absence).
