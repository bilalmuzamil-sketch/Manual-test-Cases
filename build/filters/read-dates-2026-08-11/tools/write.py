#!/usr/bin/env python3
"""The authorised writes of this pass: `update_case` on Filters cases to add
...
Rule 50 — EXHAUSTIVE then EXACT:
  * ALL THREE text fields are sent on every payload, because TestRail RE-RENDERS
    ...
    it, it is not a rich-text field, and omitting it keeps the comma-normalisation
    question out of the picture entirely. It is byte-verified unchanged regardless.
  * Every write is re-GET and byte-compared FIELD BY FIELD: the fields sent against
    the payload, and EVERY other field against the pre-write snapshot.
...
RULE41 = ("re-verified whole against the Filters specification, Confluence version 19, "
          "read live 2026-08-11 13:41Z and re-read 13:59Z — title, preconditions, steps, "
          "expected results, refs, section, type, requirement anchors, automation marker "
          "and raw-markup census all checked")
...
def run(ids, logpath):
    ...
    for n, cid in enumerate(ids, 1):
        ...
        rec = {"n": n, "op": "update_case", "case_id": int(cid), "http": st,
               ...
               "read_dates_inserted": PLAN[cid]["ops"],
               "rule41": RULE41,
        ...
        if fails:
            ...
            for t, k, a, b in fails:
                print(f"   {t} field={k}\n     expected: {a}\n     actual  : {b}")
            sys.exit(3)
        print(f"  {n:>3}/{len(ids)} C{cid} 200 MATCH ({rec['fields_compared']} fields) "
              f"atm={rec['atmstatus_at_write']} {PLAN[cid]['ops']}")
        json.dump(log, open(logpath, "w"), indent=1)
    print(f"OK — {len(log)} writes, all HTTP 200 + byte-verified MATCH")
