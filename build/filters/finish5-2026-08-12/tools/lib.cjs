...
async function ensureBarOpen(page) {
  // FIXED finish5: the old version probed for filter_chip_status specifically.  On the
  // Estimates and Completed tabs that chip is ABSENT while the bar is fully expanded with
  // its other four chips, so the probe read "closed", clicked the toggle, and COLLAPSED an
  // already-open bar -- every chip lookup below it then returned "not found" and the check
  // could not fail.  That is finish3's trap #5 recurring through a different door.
  // Probe by ANY chip, never by one particular chip.
  const n = async () => (await page.$$('[data-test-id^="filter_chip_"]')).length;
  if (await n() > 0) return { alreadyOpen: true, toggled: false, chips: await n() };
  ...
  return { alreadyOpen: false, toggled: true, nowOpen: (await n()) > 0, chips: await n() };
}
