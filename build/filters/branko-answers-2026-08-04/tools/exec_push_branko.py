#!/usr/bin/env python3
"""
Filters / Branko's 2026-08-04 answers — the EXECUTOR.

A byte-for-byte copy of the proven Rule-50 executor from
build/filters/provenance-2026-08-04/tools/exec_push.py, with ONE change: it reads
THIS pass's plan.json and writes THIS pass's exec-log.jsonl, so the two passes'
audit trails never mix.
...
# THIS PASS is a TARGETED 12-operation plan (Branko's 2026-08-04 answers), not the
# 110-case retrofit, so the plan length asserted here is the operation count.  The
# proof that the OTHER 98 cases are unchanged lives in build_plan_branko.py, which
# refuses to emit a plan if any of them would move.
EXPECTED = {'filters': 12}
...
def main():
    ...
    ap.add_argument('project', choices=('filters',))
