...
def main():
    rows = list(csv.DictReader(open(f"{ROOT}/build/printer-friendly-wo/testrail-id-map.csv")))
    iid2cid = {}
    for r in rows:
    ...
    cases = []
    for f in sorted(glob.glob(f"{ROOT}/build/printer-friendly-wo/cases/cases-*.json")):
