# QA Dashboard — refresh runbook

Refreshes the artifact at
**https://claude.ai/code/artifact/db96d3fa-b068-4044-88d2-bdc5efa52544**
with current Jira data. Keep chat output to ONE line ("Dashboard refreshed · <UTC time>")
on success; on Jira/MCP failure, skip silently.

## ⚠️ SCOPE CHANGED 2026-08-06 — THREE EPICS ONLY (QA lead's instruction)

The dashboard now shows **only** these epics, and nothing outside them is counted anywhere
on the page:

| Epic | Name | Tickets on 6 Aug 2026 |
|---|---|---|
| **SV-8785** | Filters | 38 |
| **SV-8685** | Schedule — Technician Scheduling Module | 88 |
| **SV-8582** | Reporting Suite (6 reports) | 166 |

**This replaces the project-wide "QA-relevant SV tickets" pull in step 1 below** — that pull
was filtered to QA-flagged tickets across all of project SV. The epic-scoped pull instead takes
**every** story, defect, bug and task under the three epics, whether or not a QA member is on
it, because the question being answered is "how is this epic doing", not "what is QA touching".
...
1. **`parentEpic` DOES NOT TRAVERSE on this site.** `parentEpic in (SV-8785,SV-8685,SV-8582)`
   returns **the 3 epics themselves and nothing else**. The hierarchy must be walked by hand:
   `parent in (<epics>)` gives the 144 stories/bugs/tasks, and a **second** `parent in (<those
   keys>)` gives the 148 Story Defects (a Story Defect's parent is the Story, not the epic).
   `fetch.py` loops until a pass adds nothing.
2. **The search response paginates with `nextPageToken` + `isLast` at the TOP LEVEL**, not
   `pageInfo.endCursor`. Reading only `pageInfo` **silently truncated every result set at 100**
   and cost 44 issues on the first run — the numbers looked plausible, which is what made it
   dangerous. Always verify with `searchResultMode:"count"`: 19 + 21 + 104 = 144 direct
   children, 148 subtasks under them, 292 work items total.
3. **`QA Complete` is category "In Progress" in this workflow.** Counting only
   `statusCategory == Done` reports a ticket QA has finished as still in the pipeline. gen_data.py
   sets `fin` from `FINISHED_STATUSES` and the template's `isFin()` uses it. Because the status
   change stays inside one category, **`statuscategorychangedate` does not move for it** — the
   finish date comes from the changelog (`finish-dates.json`).
...
- **⚠️ THAT ROUTINE IS CURRENTLY OFF (established 2026-08-06).** `list_triggers` shows it with
  **no `enabled` flag**, `next_run_at` **2026-07-27** (in the past) and `last_fired_at`
  **2026-07-24T15:09Z** — which is exactly why the published page sat at `asof 2026-07-24` for
  13 days. **The page no longer claims to auto-refresh**; its footer says so plainly. Before
  re-enabling it, note its prompt still tells the run to notify on **Custom Roles (SV-7388)**
  coverage changes and to pull project-wide — both wrong under the three-epic scope, so the
  prompt needs updating in the same action (`update_trigger`).
- **The date filter compares date parts, not raw timestamps** (fixed 2026-08-06). Ticket dates
  are full ISO timestamps and the range bounds are `YYYY-MM-DD`, so a plain string compare
  dropped everything on the **last day** of the range — the "Today" preset rendered zero
  tickets, and a range ending today hid today's work. `inPeriod`/`inRange` now trim with `day()`.
