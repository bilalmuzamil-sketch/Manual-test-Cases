...
doc={"type":"doc","version":1,"content":[
 ...
 table,
 heading("Evidence"),
 *media(B+"/EX1-cap-before-after-annotated.png?v=2",
    "The 50-value cap on Inventory Value, before and after (Vendor filter only, Category at \"All categories\" = 0). Below the cap every option is selectable with no notice; at 50 the unchecked options lock, the count row reads \"50 selected\" and the notice reads \"50 selected (max 50 across all filters)\" \u2014 the two match. The count is across ALL filters combined, so if some categories were also picked the vendor panel would show fewer than 50 while the notice still reads 50 (e.g. 46 vendors + 4 categories)."),
 ...
 *media(B+"/EX4-mobile-cap-annotated.png?v=2",
    "Mobile bottom sheet (390-wide viewport), Vendor filter only (Category = All, 0). The count row \"All vendors 50 selected\" and the red notice \"50 selected (max 50 across all filters)\" both read 50. Unchecked vendors are greyed and locked; a blocked add shows \"This change would exceed the 50-value limit across all filters\", and the valid 50-value draft applies via Apply Filters."),
 rule(),
...
"Customer-portal find-by-ids with >50 ids  -> 200 (exempt)")]},
 p(t("The guard counts the combined total across counted params and reports it per offending parameter — confirming the cap is across all filters, not per filter. Verified live: with only the Vendor filter used the notice fires at exactly 50 vendors, so a per-filter panel can read fewer than 50 (e.g. 46 vendors) while the notice reads 50 because other filters hold the remaining 4. X-Current-Page no longer carries the query string (2,844 bytes on a 50-vendor page).")),
