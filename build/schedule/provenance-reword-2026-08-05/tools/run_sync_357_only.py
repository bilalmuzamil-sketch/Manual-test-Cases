#!/usr/bin/env python3
"""
run_sync_2026_08_05.py — Standing-Rule-34 run sync for the THREE ACTIVE projects only
...
HERE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # this pass's folder, not the parent's
BEFORE = os.path.join(HERE, 'snapshots-before')
AFTER = os.path.join(HERE, 'snapshots-after')
BASE = 'https://shopview.testrail.io/index.php?/api/v2/'
PROJECT, SUITE = 1, 1
OUR_USER_ID = 3                     # Bilal Muzamil — Rule 38

# Rule 47: THE ONLY runs in scope. Runs 278 / 324 / 325 are out of scope and unreachable here.
# SCOPE RESTRICTED TO RUN 357 ONLY (2026-08-05, provenance-reword pass).
# The parent executor at build/testrail-run-sync-2026-08-05/tools/run_sync_2026_08_05.py
# covers all three active runs. This worker owns build/schedule/** ONLY, and other workers
# were live on Report Suite and Filters at the time, so runs 359 and 352 are removed from
# SCOPE outright rather than relied upon to have nothing to add. Every other line of the
# parent - the union-only law, the full snapshots, the by-id result verification, the
# both-directions set equality - is unchanged (Rule 27).
