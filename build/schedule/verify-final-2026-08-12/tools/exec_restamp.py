...
OUT = "/home/user/Manual-test-Cases/build/schedule/verify-final-2026-08-12/evidence/restamp-oplog.json"
payloads = json.load(open("/tmp/sched/restamp-payloads.json"))
order = [str(c) for c in json.load(open("/tmp/sched/todo.json"))]
...
for n, cid in enumerate(order, 1):
    p = payloads[cid]
    body = {
        "custom_preconds": p["custom_preconds"],
        "custom_steps": p["custom_steps"],
        "custom_expected": p["custom_expected"],
    }
    rec = {"op": n, "cid": int(cid), "was": p["old_build"] + " / " + p["old_date"],
    ...
    try:
        ...
        rec["stamp_ok"] = ("Last checked against build v3.5-65d6500 on 12 August 2026."
                           in got)
        rec["stale_gone"] = p["old_build"] not in got
