...
| **ONE SPEC CORRECTION FROM CHRIS — SBR v15 contradicts itself on the CSV headers.** His own new **`S14-R20`** (2026-07-29) says the Location column is *"included in all four exports in the same position it occupies on screen"*, but **`S14-R15`** (Summary CSV) and **`S14-R16`** (Expanded CSV) still enumerate their header lists *"in order"* **without it** — those lists date from the 2026-07-11 "Exports hardened" change and were never amended. | OTHER TEAM | **Chris Ward** (PO) | **Blocks nothing** — `S14-R20` is the newer text and wins (Rule 32), and our export cases are being fixed to it. But the contradiction sits live in the spec and **will keep re-generating this exact confusion for every reader**, exactly as it did for us. Add it to his SPEC-WATCH correction list. | 2026-07-31 |
