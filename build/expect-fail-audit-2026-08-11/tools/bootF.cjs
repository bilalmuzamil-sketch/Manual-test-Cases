// Schedule QA branch sv8785 — browser driver on the ONE established session.
// NEVER calls quick-login or switch-user (both rotate the shared sv_sso_session).
const {chromium}=require('/opt/node22/lib/node_modules/playwright');
const fs=require('fs'),path=require('path'),crypto=require('crypto');
const APP='https://sv8785.qa.shopview.com', API='https://sv8785api.qa.shopview.com';
const CK=fs.readFileSync('/tmp/qa-cookies/filters-cookie-header.txt','utf8').trim();
const UA='Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const CACHE='/tmp/assetcache-8785'; fs.mkdirSync(CACHE,{recursive:true});
...
async function boot(opts={}){
  ...
  const ctx=await browser.newContext(Object.assign({viewport:{width:1680,height:1080},ignoreHTTPSErrors:true,userAgent:UA,acceptDownloads:true},opts.ctx||{}));
  ...
  const seed=JSON.parse(fs.readFileSync('/tmp/flt-efa/seed.json','utf8'));
