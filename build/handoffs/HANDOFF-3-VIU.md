...
**a case verifying the portal feature's ABSENCE on
...
portal; it is live at `AUTOMATION: READY`. **The id that looked portal-gated was not — which is why
you never scope from the word.** **Three cases carry the literal, not four**, measured
...
Vladimir Tomovic's, so it is HANDS-OFF whatever the verdict (Rule 38)**: cited for the test, not as
work to do.
