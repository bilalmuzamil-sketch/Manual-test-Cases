...
3. **`build/BUILD-ACCURATE-WORDING-VIU-PROCESS.md`** — **the method you are executing.** Read it in
   full before anything else in this list.
4. **`build/VIU-ACCESS-METHOD.md`** — live access: egress, the three session cookies, the MITM bridge,
   the `boot2` hydration pattern.
5. **`build/APP-ACTIONS-PLAYBOOK.md`** — the indexed **STAGING ACTION RECIPES** plus **§J** (TestRail
   and API declared facts / normalisations). **Rule 27: reuse, never re-discover; append a new proven
   recipe immediately.**
6. **`build/SPEC-RELEVANCE-RECONCILIATION-PROCESS.md`** — the Rule-11 companion process; you may be
   asked for this instead of, or alongside, the VIU.
7. **`build/MISSING-TRACEABILITY-PROCESS.md`** — a standard sub-step of any VIU pass.
8. **`build/CUSTOM-ROLES-PERMISSION-VIU-PROCESS.md`** — the 4-layer live method if the pass touches
   roles or permissions.
9. **`build/RUTHLESS-USEFULNESS-AUDIT-PROCESS.md`** — required if the wording changes are extensive
   enough to count as an authoring pass (Rule 28).
10. **`build/TESTING-RUNBOOK.md`** and **`build/NO-WORK-LOSS-STRATEGY.md`**.
11. **`build/PROCESS-CATALOG.md`** — the index of every callable process.
12. **`build/OUTSTANDING-ITEMS-REGISTER.md`** — what we are already waiting on.
13. The target project's **`build/<project>/PROJECT-STATE.md`**, then its newest dated pass folder and
    every **`RECHECK-QUEUE.md`**.

**⚠️ DO NOT read `CLAUDE.md` end to end** — roughly 5,000 lines; reading it whole causes context thrash
and will cost you the session. `grep -n` for what you need. Note that **CLAUDE.md's numbered Standing
Rules stop at Rule 62**; the higher-numbered rules quoted in your skill come from the QA lead's later
instructions and are recorded in the skill file, not in CLAUDE.md.
