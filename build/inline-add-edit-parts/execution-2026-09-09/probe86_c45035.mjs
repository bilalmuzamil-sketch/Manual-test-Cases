// C45035 — the work order becomes non-editable while an inline EDIT row is open.
// Probe 83 proved C45021 on work order S9315-15893 (the alert appears and the typing survives), but
// its edit leg found "no edit control": that work order's lines carry no parts, so there is nothing
// to edit. A part is added here first, while the work order is still editable.
...
const R={};
const save=()=>fs.writeFileSync(`${DIR}/evidence/86-c45035.json`, JSON.stringify(R,null,1));
...
if (await page.$('[data-test-id=input_inline_part_description]')){
  await page.fill('[data-test-id=input_inline_part_description]','ZZAUTOTEST c45035 seed');
...
await open();
R.editOpened = await page.evaluate(()=>{const e=document.querySelector('[data-test-id^=button_edit_part_]');
...
log('edit row:', R.editOpened, JSON.stringify(R.rowBefore));
if (!R.rowBefore.open){ log('still no edit row — stopping'); save(); await s.browser.close(); process.exit(0); }
await page.fill('[data-test-id=input_inline_part_quantity]','9'); await page.waitForTimeout(1500);
...
log('C45035 toast=%s row=%s alerts=%s', JSON.stringify(R.toast), JSON.stringify(R.rowAfter), JSON.stringify(R.alerts).slice(0,260));
await page.screenshot({path:`${DIR}/evidence/86-c45035.png`, fullPage:true});
save();
// put it back
await api('/api/work-orders/change-status', {id:WOID, status:'approved'});
for (const id of LINES) await api('/api/work-orders/lines/change-status', {line_id:id, status:'authorized', workOrderId:WOID});
