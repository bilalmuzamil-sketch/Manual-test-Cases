...
const R={};
const q = async (pn)=>page.evaluate(async p=>{
  const r=await fetch(`/api/inventory/parts?search=${encodeURIComponent(p)}&pagination[rowsPerPage]=50&pagination[page]=1`,
...
R.multi = await page.evaluate(async ()=>{
  const out=[]; 
  for (let p=1;p<=6;p++){
    const r=await fetch(`/api/inventory/parts?pagination[rowsPerPage]=200&pagination[page]=${p}`,{headers:{Accept:'application/json'}});
