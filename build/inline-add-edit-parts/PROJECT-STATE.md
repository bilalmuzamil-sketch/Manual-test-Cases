...
- ⚠️ **Overlap to resolve:** C45254 (inventory part → Cost NOT enterable) vs **C45252** ("Add Part calculates the Sell Price from the cost" — its steps currently say "select an inventory/catalogue part … enter the Cost"). For an inventory part the cost is fixed, so C45252's cost-entry step should be scoped to a **special-order / custom** part. Flagged to the QA lead 2026-09-02; not changed pending his call.
