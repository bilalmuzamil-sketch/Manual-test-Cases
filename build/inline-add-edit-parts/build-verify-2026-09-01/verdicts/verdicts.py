...
44999: ('PASS', 'TB-typeahead', 'the part number field is the catalog typeahead and filters on what is typed; a no-match search answers "No results"'),
45000: ('PASS', 'TC-select', 'selecting a catalog part filled the part number (N68SL-356) and the description, and focus moved to the quantity field'),
 45001: ('PASS', 'D-overwrite', 'populated description was overwritten and kept'),
45002: ('PASS', 'TA-add-row + TC-select', 'quantity opens empty and stays empty after a catalog selection, and a save without it is refused'),
...
45004: ('PASS', 'TD-validation', 'a description with no quantity is refused and the quantity field is flagged; a part number was never required in any passing save'),
45005: ('PASS', 'TE-save', 'the part landed at the TOP of the line and the toast read "Part added" (Automated case, written under the QA lead\'s 2026-09-01 go-ahead; Rule 65 report filed)'),
45006: ('PASS', 'TE-save', 'a fresh empty row opened straight after the save with the cursor back in description'),
 45007: (None, None, ''),
45008: ('PASS', 'TF-keyboard', 'Enter from the quantity field saved the row and produced the same toast as the Save control'),
45009: ('PASS', 'TF-keyboard', 'Tech View Tab order observed as description, part number, qty, Save, Cancel, then back into the row - the documented order with no category, cost, sell price or More options; focus never left the row'),
45010: ('PASS', 'TG-close', 'Escape on an empty Tech View row closed it exactly as the close control does'),
45011: ('PASS', 'TG-close', 'the close control on a populated row raised "Discard this part?" with Keep Editing / Discard Part'),
45012: ('PASS', 'TH-click-outside', 'a click well outside the row left it open with the description and quantity intact and raised nothing'),
45013: ('PASS', 'TE-save', 'a free-typed part saved and the line shows it with the Requested status'),
45014: ('PASS', 'TA-add-row', 'the Tech View add row legend reads "Enter save & next row - Tab next field - Esc cancel"'),
...
45019: ('PASS', 'TG-close', 'an empty row closed immediately with no confirmation'),
...
45053: ('PASS', 'A-add-row + TA-add-row', 'Full View legend carries the more-options entry ("… Esc cancel - Shift Enter more options"); the Tech View legend ends at "Esc cancel"'),
 45054: ('PASS', 'G-save', 'a free-typed part saved in Full View shows on the line with the Requested status'),
45055: ('PASS', 'S-create-new + TB-typeahead', 'a no-match search offers "Create <text> as a new part" in Full View and shows only "No results" in Tech View'),
