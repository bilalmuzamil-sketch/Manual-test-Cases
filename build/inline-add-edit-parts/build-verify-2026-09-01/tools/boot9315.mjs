// boot9315.mjs — SPA hydration for the Inline Add/Edit Parts + Printer Friendly WO branch sv9315.
// Adapted from boot8218.mjs (Rule 27). The cookie name on this branch is sv_sso_session, NOT
// shopview_session — using the wrong name answers 401 sso_required and looks like dead credentials.
// The sleep guard is kept: a sleeping environment serves a ~148-character page on every route.
//
...
//   * hosts are sv9315.qa.shopview.com / sv9315api.qa.shopview.com (NO dot before `api`)
...
// The QA lead confirmed 2026-08-31 that nobody else is driving sv9315. Admin key is used first:
...
export const APP = 'https://sv9315.qa.shopview.com';
export const API = 'https://sv9315api.qa.shopview.com';
const SEED = fs.readFileSync('/tmp/qa-cookies/sv9315-live-session.txt', 'utf8').trim();
...
export async function quickLogin(key = 'admin') {
  ...
  const d = r.body?.data || {};
  fs.writeFileSync('/tmp/qa-cookies/sv9315-live-session.txt', SESS, { mode: 0o600 });
...
export async function boot(dest = '/workorders', key = 'admin') {
  ...
  // 🛑 SLEEP GUARD. sv9315 auto-sleeps and then EVERY route serves sleep.qa.shopview.com — a ~148
  ...
  if (await asleep()) {
    console.log('ENVIRONMENT ASLEEP — waking sv9315 and retrying');
    await fetch('https://fz4hhptxi8.execute-api.ca-central-1.amazonaws.com/default/toggleQaEnv', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'wake', env: 'sv9315' }),
    }).catch(() => {});
    for (let i = 0; i < 20; i++) {
    ...
    if (await asleep()) { console.log('sv9315 still asleep after waking — STOP'); process.exit(3); }
    console.log('sv9315 awake, continuing');
  }
