...
**G-R4 (page size and the document's width, all stories):** Every customer document renders A4 portrait, 210 by 297 mm. This is not per country and it is not a setting: Invoice, Estimate, Credit Invoice, the Parts Sale documents, Statement, Purchase Order and Remittance Slip all use the same page. A4 is existing production behaviour, present since these templates were written, and is confirmed here rather than introduced.

The document's width follows from the page in two steps. Both numbers are named because they sit one nesting level apart and have been mistaken for one another. A4 portrait is 595.28pt. The page margins of 30px each side and the browser's own 8px body margin leave a **718px sheet box**, which is the white sheet. The sheet's own 42px padding each side leaves a **634px content box**, which is the width every element on the document is laid out in. The 634px quoted in S12-R12 is the content box. 718px is the sheet.

A shop printing this on US Letter paper gets either a page scaled to 93.9%, because Letter is 18 mm shorter than A4, or the bottom 18 mm cut off. That is a known limitation of the existing page size rather than a defect in these documents, and it is never solved by rescaling the type (S12-R9). A per-shop page size is tracked separately as [SV-9791](https://shopview.atlassian.net/browse/SV-9791) and is out of scope here. (@chris ruling, 2026-09-07, on Mudassir's SV-9782.)
...
- **S3-R10 (what the Work Order field shows):** The Work Order field shows the work order's number exactly as it appears on the work order itself, with no document-type prefix added and no reformatting. A work order numbered "S-32136" reads "S-32136" in the field, even where the document itself is numbered "INV-S2-32136". The field's label and its position in the row are set by S3-R1. (@chris ruling, 2026-09-07; Mudassir, SV-9770.)

*\* Context note: the Terms field holds the customer's payment terms (for example "Net 30").*

**Negative cases:**

- **S3-N1:** The Work Order field is hidden in exactly two cases, and is shown in every other case.

  **(a)** The document has no work order behind it. A standalone Parts Sale Estimate or Parts Sale Invoice has none, so the field is omitted there: no label and no empty value area.

  **(b)** The work order number and the document number are character-for-character identical. The two are compared as whole strings, with nothing stripped from either and no digit run extracted: work order "S2-5468" against document "S2-5468" hides the field, while work order "S2-5468" against document "INV-S2-5468" shows it, because the strings differ.

  Case (b) is a duplicate-suppression safeguard rather than behaviour to expect on a real document. A document number carries a document-type prefix ("INV-", "EST-") that a work order number never carries, so on an ordinary Estimate or Invoice the two strings always differ and the field always shows.

  *(Revised 2026-09-07: this replaces a trailing-digits comparison. Document numbers are generated from work order numbers, so their trailing digit runs always match and the field could never appear on an ordinary document, which is what Mudassir reported in SV-9770. The trailing-against-trailing wording was itself a 2026-09-04 revision made to match the built behaviour, and the wording it replaced would have produced the behaviour this rule now restores. The Design Document shows the field and the pre-refresh documents showed it. @chris ruling, SV-9770.)*
...
- **S10-R2:** On an Estimate, the work-section heading reads "Work Summary", the headline figure is labeled "Estimated Total", and the masthead shows "Estimate date: {date}" **(a relabel, not a new element: production estimates today print "Invoice Date" and "Due date" on the estimate; this renames the issued date, drops the due-date line, and retires the quirk where a null due date rendered as today's date)**.
  ...
  The {date} is the document's issued date, the same value the Estimate and the Invoice both render, and its source is unchanged from production. On an Invoice it is that invoice's stored creation date, so it is fixed once the invoice exists. On an Estimate no invoice record exists yet, so it is the date the estimate is produced and it tracks the current date until an invoice is created: the same Estimate opened on a later day shows that later date, and two printouts of one Estimate taken on different days can carry different dates. That is existing production behaviour rather than something introduced here, it is what G-R3 describes for a document not yet issued, and it is not a defect. (Mudassir, SV-9784.)

  No validity or expiry date is shown. (The signature area's "Date" line, S9-R2, is unaffected.)
...
| --- | --- | --- | --- |
| 2026-09-07 | @chris / @claude | **S10-R2 now states where the document date comes from.** The issued date is one value rendered by both documents. On an Invoice it is the invoice's stored creation date and is fixed. On an Estimate no invoice record exists yet, so it is the date the estimate is produced and tracks the current date until an invoice is created, which means two printouts of one Estimate taken on different days can carry different dates. Recorded as existing behaviour and explicitly not a defect, so QA has something to score against. | Mudassir's SV-9784; @chris ruling, match production. Verified in history rather than assumed: the pre-refresh template at `937b83f3593`, the parent of the Phase 4 redesign `df908c864a3a` (2026-08-26), rendered `{{ data.issueDate\|date('M j, Y') }}` under an unbranched "Invoice Date:" label, so a production Estimate literally printed "Invoice Date". The refresh renamed the label and left the value untouched, so there was no behaviour to restore. The floating value traces to `Invoice.vue:612`, where `invoiceCreatedOn` is undefined until an invoice row exists, and PHP resolving an empty date string to now. No code change owed. |
| 2026-09-07 | @chris / @claude | **S3-N1 rewritten and new S3-R10:** the Work Order field shows. The hide condition is now two enumerated cases, the document having no work order behind it, or the work order number and the document number being character-for-character identical as whole strings. The second is a duplicate-suppression safeguard that ordinary numbering never reaches, because a document number carries a document-type prefix a work order number never carries, so on an ordinary Estimate or Invoice the field always shows. New S3-R10 states what the field shows, which the spec had never said: the work order number exactly as it appears on the work order, no prefix added and no reformatting. | Mudassir's SV-9770; @chris ruling. This corrects a rule I broke rather than a build defect. The trailing-against-trailing comparison was my own 2026-09-04 revision against Stefan's P4, written to match the built behaviour; because document numbers are generated from work order numbers their trailing digit runs always match, so the field could never appear on an ordinary document. The wording that revision replaced compared the whole work order number against the document number's trailing digits and would have produced the behaviour now restored. The Design Document shows the field, the pre-refresh documents showed it, and the staging evidence makes the point directly: "S-32136" and "INV-S2-32136" are not the same string, so the field was never repeating anything the customer could already read. SV-9642 is the same defect from the code side and is linked to it. |
| 2026-09-07 | @chris / @claude | **New G-R4 (Section 7):** every customer document renders A4 portrait, fixed rather than per country or a setting, confirming existing production behaviour present since these templates were written. The rule also names the two width figures that sit one nesting level apart: A4's 595.28pt less the 30px page margins and the browser's 8px body margin leaves a **718px sheet box**, and the sheet's own 42px padding each side leaves the **634px content box** that S12-R12 quotes. Printing on US Letter either scales the page to 93.9% or cuts 18 mm off the bottom; that is recorded as a known limitation of the existing page size, never to be solved by rescaling the type, with a per-shop page size tracked separately as [SV-9791](https://shopview.atlassian.net/browse/SV-9791) and out of scope here. | Mudassir's SV-9782; @chris ruling. Verified before writing: `size: A4 portrait` is present in `api/templates/invoices/invoice.html.twig` from that file's first commit and in every other customer document template (credit-invoice, imported-invoice, batch-invoice, part-sale-invoice, statement, purchase-order, remittance-slip); `report.html.twig` is the only Letter template in the system. The width chain was checked against `InvoiceDisplay.vue:148`, which derives the same 538.28pt == 718px. Also corrected on SV-9694: the masthead budget I gave Milomir used the 718px sheet box instead of the 634px content box, so his 41 dropping to 38 characters was right and my 55 dropping to 48 was wrong. Recomputed against 634px the first wrap lands at 38 characters, which means the 24px document label costs the masthead two characters and SV-9644's fix has to hold at 38, not 41. |
