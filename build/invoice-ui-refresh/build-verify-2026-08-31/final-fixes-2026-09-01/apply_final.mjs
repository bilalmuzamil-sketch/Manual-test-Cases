...
const DIR = process.env.DIR || '/home/user/Manual-test-Cases/build/invoice-ui-refresh/build-verify-2026-08-31/markers';
...
const BUILD_SENTENCE = process.env.BUILD_SENTENCE || 'Last checked against build v26.35.5-8c3cc21 on 8/31/2026.';
const TAG = MODE === 'ready' ? '' : (MODE === 'steps' ? '-steps' : '-rest');
const DONE = process.env.DONEFILE || `${DIR}/APPLIED${TAG}.jsonl`;
const FAILED = process.env.FAILEDFILE || `${DIR}/FAILED${TAG}.jsonl`;
...
const data = JSON.parse(fs.readFileSync(process.env.DATAFILE || `${DIR}/intended-blocks${TAG}.json`, 'utf8'));
const snap = JSON.parse(fs.readFileSync(process.env.SNAPFILE || (MODE === 'ready'
...
for (const cid of queue) {
  ...
  try {
    ...
    if (live.custom_atmstatus === 3) log(`C${cid} is AUTOMATED — writing under the QA lead's per-case go-ahead; Rule 65 report required`);
    ...
    // A title change goes through the same editor form as the fields (the QA lead asked for three
    // over-length titles to be shortened; they truncate on the case page at ~80 characters).
    if (rec.new_title) {
      // The edit page carries TWO elements with id="title" (the case title input and something in
      // the page chrome), so locator('#title') is a strict-mode violation. The input has a
      // data-testid, which is unambiguous.
      const t = page.locator('input[data-testid="addEditCaseTitle"]');
      await t.waitFor({ state: 'visible', timeout: 30000 });
      await t.fill(rec.new_title);
    }
    ...
    if (rec.new_title && after.title !== rec.new_title)
      throw new Error(`title not saved: wanted ${JSON.stringify(rec.new_title)} got ${JSON.stringify(after.title)}`);
    ...
    // in steps mode the marker is whatever the case already had - we are not touching it
    ...
    if (MODE === 'steps') {
      // nothing about custom_expected may move: same marker, and sentence 2 exactly as we found it
      const wantCarried = snap[cid].marker_carried;
      if (wantCarried && last !== wantCarried) problems.push(`marker changed: wanted ${JSON.stringify(wantCarried)} got ${JSON.stringify(last.slice(0,70))}`);
      const beforeS2 = (snap[cid].before_expected_s2 !== undefined) ? snap[cid].before_expected_s2 : null;
      if (beforeS2 !== null && (s2[0] || null) !== beforeS2) problems.push(`build sentence changed: ${JSON.stringify(s2[0])}`);
    } else if (MODE === 'ready') {
      if (s2.length !== 1) problems.push(`build sentence count = ${s2.length}`);
      else if (s2[0] !== BUILD_SENTENCE) problems.push(`build sentence wrong: ${JSON.stringify(s2[0])}, wanted ${JSON.stringify(BUILD_SENTENCE)}`);
    } else if (s2.length !== 0) {
