...
| [`17-REGRESSION-IMPACT-V1-TO-V2.md`](17-REGRESSION-IMPACT-V1-TO-V2.md) | **Rule 96** — the V1→V2 regression impact process (created 2026-08-26). **Runs whenever the project is a V2 / upgrade / re-work of an existing feature** (intake type **ii**), **before or alongside** V2 authoring, and it needs **no build and no cookies**. Four steps: the **V1 behaviour baseline** → the **V2 delta** (CHANGED/REMOVED/REPLACED/**SILENT**) → the **INVARIANT SET** (baseline − changed ∪ removed ∪ replaced) → **escalate the dangerous silences**. **Documents establish INTENT, product source code establishes FACT and is never a source of expectation, and a code-vs-document conflict is a PO DECISION ITEM, never a silent invariant.** Superseded V1 cases are retired/rewritten, not preserved |
| [`COVERAGE-MATRIX.md`](COVERAGE-MATRIX.md) | The completeness proof skill `01` owes — the requirement→case map, both directions |
| [`V1-BASELINE-FROM-SOURCE.md`](V1-BASELINE-FROM-SOURCE.md) | **Companion to Skill 17 (Rule 96).** The method for Skill 17 Step 1 / §3.3 — turn the CURRENT product source code into a **source-cited V1 behaviour baseline** (an invariant register + collateral-risk map + existing-coverage list + self-check, pinned to a commit SHA) that Skill 17 subtracts the V2 delta from. Feeds Skill 17; does not derive invariants or author cases itself |
