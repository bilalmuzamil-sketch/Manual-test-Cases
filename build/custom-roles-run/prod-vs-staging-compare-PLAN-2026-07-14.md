...
**Main tab "PROD-ONLY Deltas"** — one row per (staging role × capability) where **prod grants
more than staging**:
| Column | Meaning |
|---|---|
...
| Delta | PROD-ONLY / match / staging-more |
| Severity | High / Medium / Low (financial-payment-security & data-delete = High) |
| Evidence / source | spec line, migration case C#, roles-matrix row, live API capture ref |
| Notes | naming-trap/merge caveats, FE-only-gate note, NEEDS-CONFIRMATION flags |

**Additional tabs:**
- **"Summary per role"** — one row per staging role: # PROD-only deltas, highest severity,
  merged? (Y/N), mapped prod role(s), confirmation-needed? flag.
