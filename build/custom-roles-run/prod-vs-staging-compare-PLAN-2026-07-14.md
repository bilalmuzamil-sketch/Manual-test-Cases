...
**Main tab "Prod>Staging Deltas"** — one row per (staging role × capability) where **a
PRODUCTION role grants MORE than the mapped STAGING role (staging has LESS than prod)**.
**LIST EVERY such delta — spec-intended reductions are NOT filtered out; they are INCLUDED
and annotated `Yes` in the intended-reduction column so both kinds stay visible.**
| Column | Meaning |
|---|---|
| Staging Role | the new system role |
| Production role(s) mapped | the legacy role(s) feeding it (name the specific one holding the extra) |
...
| Delta (PROD-ONLY / match / staging-more) | classification of the gap |
| **Per spec — intended reduction? (Yes/No)** | **Yes** = the spec explicitly documents/accounts for this reduction (cite it); **No** = the spec does NOT account for it — these are the release-risk items needing a decision |
...
> **ALL prod>staging deltas are listed** — spec-intended reductions are included, just annotated
> `Yes` with a spec citation. The **`No` rows (reductions NOT accounted for in the spec) are the
> headline release risks** requiring a keep/change decision.

**Additional tabs:**
- **"Summary per role"** — one row per staging role: # prod>staging deltas, **count of
  "intended (Yes)" vs count of "NOT-in-spec (No)" reductions** (the **No** counts are the
  headline release risks), highest severity, merged? (Y/N), mapped prod role(s),
