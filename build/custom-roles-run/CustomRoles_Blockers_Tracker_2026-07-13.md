...
**Two-stage pass:** (1) build-accurate WORDING — all 252 core cases pushed to TestRail via `update_case` (200/200). (2) BEHAVIORAL VIU 2026-07-13 (multi-round) — role editor + product screens driven headless via boot2; data self-seeded; roles assigned to Tech then restored. No TestRail writes in the behavioral pass.

**Tally (252 core cases):** VIU-Verified 167 · Blocked-UI 74 · Deviation/Finding 11.
...
## Resume conditions
- Cookies `/tmp/custom-roles/cookies-viu-0713.env`; tooling `/tmp/custom-roles/beh0713/` (adm.mjs/boot2.mjs/drive.mjs/assign.mjs/setstatus.mjs/mkrole-api.mjs + permmap.json).
- Assign Tech (staff_id 6fb22c1b-..., email tech@shopview.com, workplace b3c8c820-...) via POST /api/staff/{id}/change; boot2('tech'); observe; RESTORE to Time Clock User a0359055-....
- mkrole-api.mjs creates roles fast: node mkrole-api.mjs NAME viewMode permCsv sfd,apar,hist (body needs organization + fePermissions[] + viewMode + crossToggles).
- GOTCHAs: /parts/part-sales direct-goto redirects to /workorders (use in-app nav-click); /roles-permissions/{id}/edit route not guarded (FE-gap, not a case fail).
