...
| **ShopView `sv8582` (Report Suite)** | same, then retried clean, plus the api host | 🔴 **UNREACHABLE** | `sv8582.qa.shopview.com` → **curl exit 56, HTTP 000** on two attempts; `sv8582api.qa.shopview.com` → same. **CORRECTED against the build-verify lane's committed evidence (`build/build-verify-session-2026-08-21/PREFLIGHT.md`, commit `a106e9e2`): the underlying failure is `CONNECT tunnel failed, response 502` — the same 502 register row R4 records, not a new condition. My `HTTP 000` is curl reporting no HTTP response of its own because the proxy tunnel never opened; it is the same fault seen one layer down.** Two lanes, two independent probes, one verdict. | 05:56Z, 05:58Z |
