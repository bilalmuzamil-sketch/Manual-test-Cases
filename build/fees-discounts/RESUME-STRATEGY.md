...
- `testrail-import/fees-discounts-v1-testrail-import.csv` (+ `.xlsx`) — all 182
  cases, generated by `build/fees-discounts/gen_import.py`. They are **VIU-word-free
  and feature-flag-free by user rule** (internal `viu_status`/`notes`/`design_ref`
  are not emitted; the literal "feature flag" phrase is reworded to "Fees & Discounts
  feature"). Sections are leaf names; References carry the spec/story reference only.

Finalize in TWO phases:

1. **Phase 1 — VIU when the env is ready.** Once staging is redeployed with the
   feature, run VIU against the live app, then **REGENERATE both deliverables**:
   re-run `gen_import.py` (rebuilds the TestRail import CSV **and** the Excel
   workbook in one pass). Fix any case where live behavior differs from the spec.
2. **Phase 2 — dev answers.** Any open questions still remaining after VIU (see §4
   and `requirements.md §14`) get sent to the user → dev answers → folded back into
   the cases, then **regenerate the import + workbook again**. Only the regenerated
   post-VIU + dev-answered files are the FINAL TestRail import.
