...
> - Current version: v11
> - Last updated: 2026-07-21T07:12:55.826Z by Chris Ward
...
> _* Context note: for the Custom range the literal word "custom" is used; the actual start and end dates are not put in the filename._
  ...
  * **S14-R10:** The Margin % cell is a plain number to one decimal with no percent sign (for example, 64.7); it is empty when the row's Subtotal is zero or below. 

> _* Context note: the CSV is flat — Customer then Invoice rows. The on-screen asset layer is not represented in the main CSV; the asset breakdown is available on screen._
...
**Requirements:**
  ...
  * **S15-R9:** The header date range shows the start and end dates in the format "Mon D, YYYY," joined by an em dash — for example, "May 1, 2026 — May 31, 2026."

> _* Context note: the header range uses this comma form on purpose; the body Date cells use "Mon DD YYYY" (S15-R17). The header-versus-cell difference is intentional._
