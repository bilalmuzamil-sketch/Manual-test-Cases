# SBC (Sales By Customer) Report — COMPLETE spec (verbatim-structured)
...
> - **Report:** SBC (Sales By Customer) Report.
> - **Canonical spec URL (Confluence):**
>   https://shopview.atlassian.net/wiki/spaces/~712020aa00b8d6a71f4259891982a304227c20/pages/577634305/SBC+Sales+By+Customer+Report
...
> - **Spec doc metadata (from the export header):** Epic TBD · Owner TBD · Status = In review — 2026-07-16 · Branch TBD · latest change-log entry 2026-07-21 (Milan review resolution: server-backed type-ahead Customer filter, all-customers state, 10k export cap). Export filename `SBCSalesByCustomerReport_2.doc` (revision `_2` — second export of this spec; the change log confirms it carries the 2026-07-21 round).
...
- The report is a three-level tree: Customer → Asset → Invoice. A customer row expands to that customer's assets (the vehicles the work was done on); an asset row expands to the individual invoices for that vehicle. Work with no vehicle (counter / parts sales) collects under a per-customer "Parts Sales" bucket.
...
- The user can download the report as CSV or PDF, or print it, from a single overflow menu in the toolbar, and can show or hide individual columns from a column selector.
...
- **S2-R9:** When the report is opened with both a saved view and a date range in the page link, the saved view is applied and the link value is ignored.  ** Context note: the remembered view (Story 6) was given priority over the link so a returning user always lands on their own last view. A shared link therefore restores the sender's range only for a recipient who has no saved range of their own for this report.*
**Negative cases:**
...
- **S6-R3:** The saved view does not include the Customer filter's type-ahead search text, expansion state, or scroll position.  ** Context note: the Customer filter's search text is a transient way to find a name in the dropdown, not a saved setting — only the resulting customer selection is saved (S18-R8); expansion is treated as fresh each visit so a returning user is not faced with rows expanded against data they have not looked at yet.*
...
- **S8-R6:** A customer's work with no vehicle collects under a single row labeled "Parts Sales," always sorted last among that customer's assets.  ** Context note: a service work order always has a vehicle, so no-vehicle work is treated as a parts sale.*
...
- **S8-R7 (asset label — base):** The asset label starts with the vehicle's year, make, and model.
- **S8-R8 (asset label — identifier suffix):** After the base, the label adds the most specific identifier available, in this priority order: (1) " · Unit {unit}"; else (2) " · {plate}"; else (3) " · VIN …{the last 8 characters of the VIN, or the whole VIN if it is shorter than 8}"; else (4) no suffix.
- **S8-R9 (asset label — no year/make/model):** When the vehicle has no year, make, or model, the label is the VIN on its own.
- **S8-R10 (asset label — nothing on file):** When the vehicle has no year, make, model, or VIN, the label reads "Unknown Asset."
...
Download the on-screen report as a CSV file.
**Design:** See Story 20 **Jira:** TBD
**Prerequisites:**
- User is on the Sales By Customer report.
**Requirements:**
- **S14-R1:** CSV export is started from the "Download (CSV)" item in the toolbar's overflow menu — the same menu that holds "Download (PDF)" and "Print." There are no separate always-visible export buttons.
- **S14-R2:** The overflow menu item reads "Download (CSV)."
- **S14-R3:** The CSV is generated on the server from the active date range, Product Type, location, Customer filter (Story 18), and sort, and contains the customers matching those filters (S18-R7).
- **S14-R4:** The file is named for the report and the active date range, using this map from range label to filename: Today → sales-by-customer-today.csv; Yesterday → sales-by-customer-yesterday.csv; This Week → sales-by-customer-this_week.csv; Last Week → sales-by-customer-last_week.csv; This Month → sales-by-customer-this_month.csv; Last Month → sales-by-customer-last_month.csv; This Year → sales-by-customer-this_year.csv; Last Year → sales-by-customer-last_year.csv; This Quarter → sales-by-customer-this_quarter.csv; Last Quarter → sales-by-customer-last_quarter.csv; Custom → sales-by-customer-custom.csv.  ** Context note: for the Custom range the literal word "custom" is used; the actual start and end dates are not put in the filename.*
- **S14-R5:** The downloaded file is plain comma-separated text with a .csv extension and opens as rows and columns in a spreadsheet; it is not an .xlsx workbook and not a JSON file.
- **S14-R6:** The CSV has twelve columns in this exact order: Customer, Invoice #, Date, Inv. Hrs, Labor Invoiced, Labor Margin, Parts Invoiced, Parts Margin, Shop Supplies, Margin, Margin %, Subtotal.
- **S14-R7:** On customer rows the Invoice # and Date cells are blank.
- **S14-R8:** On invoice rows the Customer cell is blank and the Invoice # and Date cells are filled.
- **S14-R9:** Customer names in the CSV are plain — the "(N)" invoice count is not included.
- **S14-R10:** The Margin % cell is a plain number to one decimal with no percent sign (for example, 64.7); it is empty when the row's Subtotal is zero or below.  ** Context note: the CSV is flat — Customer then Invoice rows. The on-screen asset layer is not represented in the main CSV; the asset breakdown is available on screen.*
- **S14-R11:** Dates export as mm-dd-yyyy — for example, 05-14-2026 — matching the ShopView-wide CSV date format.
- **S14-R12:** Currency values export as plain numbers with no dollar sign and no thousands separators.
- **S14-R13:** The CSV has no color. The signed Inv. Hrs value still conveys direction.
- **S14-R14:** The CSV export is capped at 10,000 data rows — the combined count of customer rows and invoice rows, not counting the header row or the totals row — counted on the server against the active date range, Product Type, location, Customer filter, and sort. When the result would exceed the cap, the server does not generate the file, no download starts, and the user is shown an error toast: "This export is too large to generate. Narrow the date range or filters, then try again."  ** Context note: the cap is a guardrail against runaway exports; it is enforced server-side against the same filters and sort that produce the on-screen report, so the count the cap checks is the count the file would contain.*
**Negative cases:**
- **S14-N1:** If the export fails, the user is shown an error toast: "CSV export failed."
**Edge cases:**
- **S14-E1:** While the export is in progress, the menu's CSV action shows a loading state and is non-interactive.
...
- User is on the Sales By Customer report.
**Requirements:**
- **S15-R1:** PDF download is started from the "Download (PDF)" item in the toolbar's overflow menu.
- **S15-R2:** The overflow menu item reads "Download (PDF)."
- **S15-R3:** The PDF is generated on the server from the active date range, Product Type, location, Customer filter (Story 18), and sort, and contains the customers matching those filters (S18-R7).
- **S15-R4:** The file follows the same range-to-filename map as the CSV (S14-R4), with a .pdf extension — for example, sales-by-customer-this_month.pdf.
- **S15-R5:** The PDF is A4 landscape with 25px margins on all sides, using the application's standard font.
- **S15-R6:** The footer has a centered "Software Powered by ShopView" label and a right-aligned page number, "Page X of Y."
- **S15-R7:** The PDF header is two columns: a text block on the left (70% width) and the organization logo on the right (30% width).
- **S15-R8:** The header text block shows, in order: the report title "Sales By Customer Report"; the organization name; the date range; and the filter summary line "Product Type: {value}" where value is "Parts & Service," "Parts only," or "Service only."
- **S15-R9:** The header date range shows the start and end dates in the format "Mon D, YYYY," joined by an em dash — for example, "May 1, 2026 — May 31, 2026."  ** Context note: the header range uses this comma form on purpose; the body Date cells use "Mon DD YYYY" (S15-R17). The header-versus-cell difference is intentional.*
- **S15-R10:** Every date range is bounded, so the PDF header date range always shows a start and end date (per S15-R9); there is no unbounded-range wording.
- **S15-R11:** The location filter is not shown in the PDF header; it scopes the data only.
- **S15-R12:** The organization logo is embedded in the PDF, not loaded from a network address, so it renders offline.
- **S15-R13:** The logo scales to fit its area without distortion, pinned to the top-right.
- **S15-R14:** The logo is chosen in this order: (1) the organization's uploaded logo; (2) the bundled ShopView logo when none is uploaded; (3) no logo.
- **S15-R15:** When no logo is available, the logo column is not rendered and the text column fills the full width.
- **S15-R16:** The PDF body table has twelve columns in the same exact order and labels as the CSV (S14-R6). Like the CSV, the PDF is flat — Customer then Invoice rows — with no asset layer.
- **S15-R17:** Dates in the PDF body show in the format "Mon DD YYYY" — for example, "May 14 2026."
- **S15-R18:** The Date cell is blank on customer rows in the PDF, matching the screen.
- **S15-R19:** The Margin % cell shows an em dash when the row's Subtotal is zero or below, matching the screen.
- **S15-R20:** The Subtotal column is bold in the PDF — header, every row, and the totals row.
- **S15-R21:** Inv. Hrs in the PDF uses the same signs and coloring as on screen (S12-R3, S12-R4, S12-R5), with green rendered as #21ba45 and red as #c10015.
- **S15-R22:** The PDF export is capped at 10,000 data rows — customer rows plus invoice rows, not counting the header row or the totals row — counted on the server against the active date range, Product Type, location, Customer filter, and sort. When the result would exceed the cap, the server does not generate the file, no download starts, and the user is shown an error toast: "This export is too large to generate. Narrow the date range or filters, then try again."
**Negative cases:**
- **S15-N1:** If the export fails, the user is shown an error toast: "PDF export failed."
**Edge cases:**
- **S15-E1:** While the export is in progress, the menu's PDF action shows a loading state and is non-interactive.
...
- User is on the Sales By Customer report.
**Requirements:**
- **S16-R1:** Print is started from the "Print" item in the toolbar's overflow menu — the third item, after "Download (CSV)" and "Download (PDF)."
- **S16-R2:** The overflow menu item reads "Print."
- **S16-R3:** Activating Print fetches the report's PDF — the same content as the PDF download (S15).
- **S16-R3a:** Print then opens the browser's native print dialog in the current tab.
- **S16-R3b:** No new browser tab is opened.
- **S16-R3c:** The page behind the print dialog is unchanged.
- **S16-R4:** The Print action shows a loading state while the PDF is being fetched and is non-interactive during that time.
- **S16-R5:** The Print action is disabled while the report table is still in its initial loading state.
- **S16-R6:** Because Print fetches the same server-generated PDF as the PDF download (S16-R3), the PDF row cap (S15-R22) applies: when the result would exceed the cap, no PDF is fetched, the print dialog does not open, and the user is shown an error toast: "This export is too large to generate. Narrow the date range or filters, then try again."
**Negative cases:**
- **S16-N1:** If the PDF request fails, the user is shown an error toast: "PDF generation failed."
...
- **S20-R10:** The totals row uses the white surface (#ffffff), or the dark surface in dark mode, with a top border and bold text.  ** Context note: the totals row was set to white on purpose, not the tinted background, to match Technician Efficiency, the suite's visual reference.*
...
- **S21-R3:** At 1024px width and above, the toolbar uses the desktop single-row layout.  ** Context note: the two-row layout keeps a filter dropdown's expand arrow from overlapping the action controls.*
...
| Print PDF fails | "PDF generation failed." | Error toast, dismissed by the user |
| An export (CSV, PDF, or Print) exceeds the 10,000-row cap | "This export is too large to generate. Narrow the date range or filters, then try again." | Error toast, dismissed by the user; no file is generated and no download or print starts |
| CSV export starts | (no toast) | The overflow menu's CSV action shows a loading state until the export resolves |
| PDF export starts | (no toast) | The overflow menu's PDF action shows a loading state until the export resolves |
| Print starts | (no toast) | The overflow menu's Print action shows a loading state until the PDF resolves |
