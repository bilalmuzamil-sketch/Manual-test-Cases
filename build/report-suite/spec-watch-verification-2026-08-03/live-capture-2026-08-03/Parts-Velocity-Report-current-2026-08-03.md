...
> **VERBATIM LIVE CAPTURE — current Confluence spec**
> - pageId: 620888066
...
>   (REST storage-format -> html2text). Compare by requirement text, not by byte diff.
> - The 2026-07-31 captures are RETAINED unchanged in `../../spec-current-2026-07-31/`.
...
Parts managers have no quick way to see which stocked parts are driving sales versus sitting idle, and no visibility into how often special-order parts are being sourced for jobs. Without this visibility, reorder decisions rely on gut feel: fast-moving parts stock out, slow-moving parts tie up capital on the shelf, and frequently-ordered special-order items that could be stocked go unnoticed. The Parts Velocity report ranks each part by its sales activity — across both inventory-held and special-order parts — and pairs that movement signal with per-part profitability (revenue, cost, margin), giving managers a single screen to identify what to stock more aggressively, what to monitor, which special-order items are ordered often enough to consider carrying, and which parts actually make money.
...
- A report under **Reports → Parts** shows all parts (inventory and special-order) ranked by their sales activity over a selected date range. The **Parts** section under Reports is new — this feature introduces it (see S1-R1).
- Parts are split into two types: **Inventory** (stocked on the shelf, tracked in the inventory system) and **Special Order** (parts sourced from a vendor on a per-job basis).
...
- Each row carries both **movement** metrics (Units Sold, Units Returned, Sold (WO), Sold (Parts Sale), Demand, Last Sale) and **profitability** metrics (**Unit Cost, Sell Price, Revenue, Margin, Margin %**), plus inventory-stocking metrics (On Hand, Turns / Yr, Min, Max) for inventory parts.
...
- Special-order rows always show `—` for inventory-only metrics (On Hand, Turns / Yr, Min, Max) because special-order parts are not held in inventory.
...
- **Special-order inventory metrics are always null.** Special-order parts are not stocked, so On Hand, Turns / Yr, Min, and Max are not applicable and always render as `—`.
- **Units Returned counts initiated returns, net of cancellations.** It is sourced from part-return records — created by a work-order part return or a parts-sale credit, not from invoice voids: each return adds its quantity; each cancelled return is excluded. Counted the same way for both inventory and special-order parts. Core-charge returns are excluded.
- **Core parts are excluded** from both the inventory and special-order result sets (S5-R1, S5-R2).
- **A part is shown when it has any real in-window activity or stock.** An inventory row is dropped only when it has **no stock movement, no billed revenue, and less than one unit on hand** — so a part that made money in the window is never dropped for lack of stock movement (S3-N1). Special-order rows are built solely from actual vendor requests in the window, so a never-requested special-order part simply produces no row.
...
- **Special Order part** — A part sourced from a vendor on a per-job basis via a part request. Not stocked; no on-hand quantity. One report row per special-order part, merged across the selected locations.
- **Type** — The classification of a report row: `Inventory` or `Special Order`.
- **Demand** (a.k.a. demand frequency) — The number of separate stock-decrement transactions (for inventory parts) or vendor part requests (for special-order parts) that included a given part in the selected date range. Each transaction counts once regardless of quantity. A later reversal of a sale does not add to, or subtract from, the count (S5-R4). This is the default ranking signal.
- **Units Sold** — For an inventory part, the net quantity moved off the shelf via stock-movement events in the window (net of invoice reversals; can be zero or negative if reversals exceed sales). For a special-order part, the total quantity across its in-window vendor requests (net of reversed/voided sales, per S5-R4b). This is a *stock-movement* figure and can differ from the billed units behind the profitability columns for inventory parts (see §3, S5-R7).
...
- **Turns / Yr** (annual turns) — How many times the on-hand stock of an inventory part sells through in a year, annualized from the in-window sales rate. Not applicable to special-order parts.
...
- Each part is uniquely identified within a workplace as either an inventory part (inventory rows) or a special-order part (special-order rows).
- Work orders are one of two types — **Service** or **Parts** — which is why Sold (WO) (Service) and Sold (Parts Sale) (Parts) together cover every billed work order.
...
- **S2-R12:** When the Location filter (S2-R9) resolves to **more than one** location in scope, the table shows a per-row **Location** column identifying each row's location; when a single location is in scope the column is **hidden**. Per-row values follow S3-R10.
...
- **S3-R1a:** **Row model across locations.** An **inventory** part is a per-location stock record: the same part number stocked at N of the selected locations produces N inventory rows, each carrying that one location's On Hand / Min / Max (never summed). A **special-order** part produces a single row, with its movement and profitability values **summed across** the selected locations. (Money columns being additive, the special-order merge reconciles cleanly.)
- **S3-R2:** Rows load ranked by **Demand**, descending (see Story 5 for the metric). Inventory and special-order rows are ranked together; rows with equal Demand keep inventory-before-special-order order. This initial Demand-descending order is the **active sort** — the Demand header shows the descending indicator (S3-R3), and it is what S4-R6 persists as the saved sort until the user clicks another header.
...
These tooltip strings are deliberate plain-language summaries for the manager; the authoritative, exhaustive metric definitions are in S5-R4 / S5-R4a (which, e.g., also include Paid work orders and — for special-order rows — vendor requests).
...
- **S4-R3:** The **6** columns hidden by default (available via the picker) are: **Units Returned, Sold (WO), Sold (Parts Sale), Turns / Yr, Min, Max**.
...
- **Work-order date** — the work order's end date. Special Order sourcing, the Sold (WO) / Sold (Parts Sale) metrics, **and the profitability columns' billed-line window** are all anchored to this date. (Only Invoiced/Paid work orders count at all — see Assumptions; this defines *which* of their dates does the windowing.) Inventory **Units Sold and Demand** are windowed instead by the inventory-movement event date — so for an inventory part the movement anchor and the billed/work-order anchor can differ; that is intended (see S5-R7). **Last Sale is not windowed at all** — it is measured over all-time history (S5-R4), scoped to the selected location(s).
...
- **S5-R2: Special-order data source.** Special-order metrics are derived from vendor-sourced part requests (carrying a catalogue part reference, and **not a core**) on work orders with status **Invoiced** or **Paid** whose work-order date falls within the window — grouped so each special-order part is one row (summed across the selected locations, S3-R1a).
- **S5-R3: Returns data source (Units Returned).** Units Returned is sourced from part-return records — the record written when a return is initiated, via a work-order part return or a parts-sale credit — NOT from inventory movements or invoice reversals. Cancelled returns are excluded; core-charge returns are excluded; the count is scoped to the current location(s) and to returns whose initiation date falls within the window. Inventory rows attribute returns to the inventory part; special-order rows attribute them to the special-order part.
    ...
    - **Date anchor:** returns are dated by their initiation date, while sales are dated by their own sale event date (per S5-R1 for inventory and S5-R2 for special-order). Because each uses its own natural event date, a return and its originating sale can fall in different report windows. Units Returned for a part is independent of the work order's current invoice status.
...
| Column | Inventory part | Special Order part |
| --- | --- | --- |
...
| **Units Returned** | Total returned quantity from part-return records attributed to this inventory part (see S5-R3). | Total returned quantity from part-return records attributed to this special-order part (see S5-R3). |
| **Sold (WO)** | Total part quantity on **Service**-type invoiced/paid work orders whose work-order date is in the window, net of reversed/voided sales (S5-R4b). | Same calc, on special-order requests. |
| **Sold (Parts Sale)** | Total part quantity on **Parts**-type invoiced/paid work orders whose work-order date is in the window, net of reversed/voided sales (S5-R4b). | Same calc, on special-order requests. |
...
| **Last Sale** | Whole days from today to the part's most recent sale at the selected location(s), over **all-time** history — not limited to the window. | Whole days from today to the special-order part's most recent vendor-sourced invoiced/paid request at the selected location(s), over **all-time** history. |
...
| Column | Formula (both inventory and special-order) | Null rule |
| --- | --- | --- |
...
- **S6-R1:** A **â‹¯** overflow button in the toolbar (leftmost in the toolbar's action cluster) opens an export menu with two items, in this order: **Download (PDF)** and **Download (CSV)**.
...
- **S6-R3:** Both exports include only the columns currently enabled, in the **canonical on-screen order** (S4-R4) — the export mirrors the visible grid. *(Build-delta: the current export builds its column order from the user's visible-columns array, which appends a re-enabled hidden column at the end rather than in its canonical slot; align it to the canonical order so the export column order matches the screen.)*
...
- **S7-R6:** The report supports dark mode; page background, toolbar, and cells use their dark-mode equivalents. The grey â“˜ info icon uses a token that meets at least a 3:1 contrast ratio against the cell background in both light and dark mode.
...
- **S7-R8:** When shown (S2-R12 / S3-R10), the per-row **Location** column renders as the **leftmost** column, before Type, using the suite's standard column treatment so its placement matches the same column on the other reports in the suite. The Location filter control (S2-R9) keeps a **constant width** regardless of the selected label (a single location name, several, or "All Locations"), so the toolbar layout does not shift when the selection changes.
...
*\* Casing note: the success toasts use uppercase *`(CSV)`* / *`(PDF)`* while the failure toasts use lowercase *`(csv)`* / *`(pdf)`*. This is the shipped wording and is documented as-is; a future copy pass may normalize the casing.*
