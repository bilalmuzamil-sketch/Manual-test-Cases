...
| 4 | [SV-8821](https://shopview.atlassian.net/browse/SV-8821) | Creating an invoice from a completed work order fails with a server error | **⚠️ RECLASSIFIED 2026-08-04 → API-RELATED** *(was "USER-FACING — but technically characterised")* | **This row was WRONG, and it was the pack's worst call.** It claimed *"the screen route fails too, so it is a user-facing defect"*. Re-reproduction (`repro-sv8821/`) proved the opposite: with no contact on the work order the **Finance tab is disabled** with the product's own message *"Please select a contact for the asset"* and **no Create Invoice button exists on screen**; with a contact set, pressing it returns **201**. **The screen route never fails.** The residual — a generic 500 instead of a clear 400 — is reachable **only** by calling the endpoint directly, so it meets the Rule-51 reachability test for API-RELATED and should have been asked about before filing. | **OBSOLETE / Done** — closed 2026-08-04T01:14:57−0500, **not by us**; left untouched. Description corrected + comment added 2026-08-04. |
