...
| **FILED AS** | **SV-8820** — https://shopview.atlassian.net/browse/SV-8820 |
...
- **Observed:** 2026-08-04
- **Server date at observation:** 2026-08-04. Nightly stock history on this organisation begins around 2026-08-01 (one to two days deep)
- **Report affected:** Inventory Value
...
The report answers with the **day after** the one you chose — always exactly one day out.

Ask for 3 August and it shows 4 August. Ask for 31 July and it shows 1 August. Ask for 31 January and it
shows 1 February.

Nothing on the screen suggests anything is wrong. The totals look sensible and the only clue is the small
"As of" line under the report title, which shows the shifted date instead of the date you asked for.

There is a knock-on effect: that "As of" line is only meant to appear when the report has had to fall
back to an earlier recorded day than you asked for. Because the date always lands one day *later*, the
report can never land earlier, so the message's real purpose can never happen — and instead it is shown
all the time.
...
![inventory-value-as-of-line.png](inventory-value-as-of-line.png)

*The Inventory Value report on its default view. Note the **"As of 08/04/2026"** line beside the report title: the indicator is showing even on the ordinary current view, which is the knock-on effect described above.*
