...
## PV RE-VERIFY SWEEP 2026-08-19 — interim `<br>` on the 36 manual PV cases + 13 Automated held
The 36 manual (atm=1) PV cases not already freshly build-verified are now re-stamped on
**`v3.8-d0e135e`** in the **interim `<br>` line-break format** (TestRail API markdown-wrap regression
still active; QA lead accepted `<br>`). Tester-facing text stores literal `<br>` between numbered lines,
`<br><br>---<br>` before the Rule-54 provenance, and `<br><br>` before the `AUTOMATION:` marker — this is
**cleanup debt**, to be demarked to plain once the TestRail API is fixed.

**13 PV Automated (atm=3) cases remain HELD (0 writes)** pending ratification — see
`PV-SWEEP-HELD-AUTOMATED.md`. Marker changes wanted: **C30346 + C30353** lift `Not available` → `READY`
(features built); **C30352** strip stale SV-8938 expect-fail (confirm PO position first); **C30328
