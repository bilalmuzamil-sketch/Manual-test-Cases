// UI re-pin of the Report Suite cases the API cannot safely write — TestRail web editor
...
//
// Per case: Rule-71 gate (re-GET custom_atmstatus; 3 => SKIP), read the rendered page BEFORE,
// edit, Save, then verify by API re-GET AND by re-reading the rendered page.
import { chromium } from '/opt/node22/lib/node_modules/playwright/index.mjs';
import fs from 'fs';

const DIR = '/home/user/Manual-test-Cases/build/report-suite/repin-2026-08-28';
const DONE = `${DIR}/UI-REPINNED.jsonl`;
const FAILED = `${DIR}/UI-FAILED.jsonl`;
const PLAN = `${DIR}/ui-write-plan.json`;
...
for (const cid of queue) {
  ...
  try {
    ...
    if (before.custom_atmstatus === 3) {
      fs.appendFileSync(FAILED, JSON.stringify({ cid, skipped: true, reason: 'AUTOMATED (custom_atmstatus=3) — held for the QA lead, Rules 65/71', at: new Date().toISOString() }) + '\n');
