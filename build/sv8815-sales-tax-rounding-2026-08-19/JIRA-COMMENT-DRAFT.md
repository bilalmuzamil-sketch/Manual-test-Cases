> **FINAL: one standalone comment is now the record - comment 75278**, posted 2026-08-20 01:37 -0500,
> **19 annotated exhibits**, opening with "OVERALL QA STATUS: PASSED". Source of truth for its content:
> `evidence/adf-single.json`, generated by `evidence/build_single.py` (which lifts the AC table, the
> open-questions table, the 30 original check rows and the 14 original captions straight out of the
> earlier write-up so no figure was retyped).
>
> Written as a clean QA result at the QA lead's instruction - no correction or "was wrong / now right"
> framing anywhere in it; a tone gate over the rendered text confirmed that before posting. The QA lead
> is deleting the two earlier comments (75272 and 75277), so 75278 stands alone and repeats everything
> they carried.
>
> Verified after posting by reading it back from Jira: 19 of 19 `mediaSingle` nodes stored as
> `type: "external"`, all 19 URLs confirmed HTTP 200 beforehand, and the build marker re-read
> immediately before the write (`v3.8-1f5fb3c`, etag a9e66ecc2174eb6d889221f4d976ef24 - unchanged).
