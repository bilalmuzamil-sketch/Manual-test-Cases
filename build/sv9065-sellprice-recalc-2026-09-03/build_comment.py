...
checks=[
 ...
 ("If the pricing request fails while changing Category (simulated failure - see reproduction): the notice \"Could not recalculate the Sell price for this category.\" appears, the spinner clears, and the previous Sell Price stays (no silent fake recalculation).","PASSED"),
]
 ...
 *media(B+"/04-checkC-failed-request-error.png","Simulated failure (I deliberately forced the pricing call to fail - see reproduction). While changing Category the notice \"Could not recalculate the Sell price for this category.\" appears, the spinner clears, and the Sell Price stays at the previous $170.00 rather than pretending to be Uncategorized's $222.22."),
 rule(),
 h("How to reproduce"),
 ...
 p(strong("Failure path (simulated) - "),t("This one needs the pricing request to fail, which does not happen in normal clicking, so a tester simulates it: open the browser DevTools -> Network tab -> right-click the \"calculate-sell-price\" request -> \"Block request URL\" (or set the network to Offline), then change the Category. You then get the \"Could not recalculate the Sell price for this category.\" message, the spinner stops, and the previous Sell Price stays put. Full disclosure: in my run I forced this by making that one call return an HTTP 500, purely to check the app's error handling from PR #2907 - it is not a real defect and not something a normal user hits.")),
