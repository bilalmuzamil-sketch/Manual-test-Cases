...
- **🛑 EVERY TEST CASE SHIPS WITH ITS OWN `spec.ts` (115; QA lead 2026-09-22: *"going forward we would need the specs for each test case too"*, clarified *"Specs = spec.ts file"*).** A case is not finished when its steps are written — it is finished when a Playwright spec sits beside it. **One spec per case, named C-id FIRST** so a red result points at a case without cross-referencing. It carries: the **source** it asserts (same document, version and section as the provenance line, 54/114) · the **traps** that would make the measurement lie, commented where they bite · a **positive control** for any negative assertion (104) · `[expected to fail: SV-xxxxx]` on anything reproducing a known fault. **NEVER write a spec for a case that cannot be fairly automated** — name it in the README with the reason, because a test that goes red for its own reasons is worse than no test. **Never build a state the product itself refuses to create** (the part-sales role, SV-10278, withdrawn for exactly that). Lives in `build/<project>/e2e/`; **secrets never enter a spec (82)**. **🛑 AMENDED SAME DAY — PER PROJECT, AS A BACKFILL, AND IT MUST NOT SLOW US DOWN (his words).** Six mechanics, not suggestions: **the probe you already write IS the spec** (write it as `spec.ts` first, not a throwaway) · **never automate a check not yet run by hand** · **table-driven — one named test per case, NOT one file per case** · **the backfill rides along with work already happening on that project, never a sweep** · **if writing it costs more than running the check by hand three times, don't — note it manual-only in the README** · **a flaky spec is deleted after two attempts, never nursed or skipped.** If a status ever reads *"spent the day writing specs"*, the rule has failed and is raised with him. Worked example and the twelve encoded traps: `build/global-search/e2e`. Full text: `RULES-61-ONWARD.md` rule 115 and its same-day amendment.
...
- **🛑 THE EXPECTED BEHAVIOUR IS THE SOURCE'S EXACT QUOTE, AND IS NEVER CHANGED (114; QA lead
  2026-09-21: *"the expected behavior should be the exact QUOTE from the specs/source, that MUST NOT
  ever be changed"*, after *"you MUST NEVER change the expected behavioer ever"*).**
  **(a) IT MUST BE THE QUOTE.** Expected Results carry the source's **own sentence, verbatim**, with
  its document, version/id and section — never our restatement of it. A paraphrase inside Expected is
  a defect in the case, and **a paraphrase presented as the source's words is worse** (both C55736 and
  C55737 cited a §9 sentence the PRD does not contain). Where no source sentence covers the behaviour,
  the case has **no Expected to write** — raise it, do not invent one (64).
  **(b) IT IS NEVER CHANGED.** You TEST AGAINST the Expected; you do not edit it to suit anything. **Not** because the build cannot
  ...
  **Blocked** with the reason. Expected looks wrong against the source ⇒ **REPORT it and leave it**.
  ...
  not.** **🛑 114(c), 2026-09-25 — ONE EXCEPTION, AND IT IS NEVER INFERRED:** he tells you IN THAT
  CONVERSATION to change the Expected of cases he NAMES. Then those cases only. **A ruling about how the
  product should behave is NOT that instruction** — record it as the run result, report that the case
  text diverges, leave the field. No precedent, no similarity to an earlier authorised change, no
  handoff and no judgement of your own lifts it. Full text: `build/rules/RULES-61-ONWARD.md` rule 114,
  carried by skills 06 and 09.
...
- **🛑 PROVE A ROLE IS DEFAULT WITH "RESET TO TEMPLATE" — THE SAVE BUTTON IS THE TELL (118; QA lead
  ...
  session had changed it: press Save and RE-RUN whatever was already measured on it.** **Save stays
  DISABLED ⇒ it was already default.** Editing a role for a test stays permitted (107); this bites only
  when the check needs the role's DEFAULT permissions, which are PROVED, never assumed. A system role
  saves nothing (L0193). Full text: `build/rules/RULES-61-ONWARD.md` rule 118.
...
## 2 · THE RULE INDEX — all rules to 110, and where each one lives
...
| **114** | THE EXPECTED BEHAVIOUR IS THE SOURCE'S **EXACT QUOTE**, AND IS NEVER CHANGED — verbatim sentence + document + version + section, never our restatement; you test AGAINST it. Cannot run it ⇒ Blocked; looks wrong ⇒ report and leave it. Supersedes Rule 106's correct-the-Expected clause. **114(c), 2026-09-25: the bar lifts ONLY when he explicitly tells you, in that conversation, to change the Expected of cases he names — never by inference, and a ruling about BEHAVIOUR is not such an instruction (record it as the result, report the divergence, leave the field)** |
...
| Standing Rules 61 onward (full text — 61–110 today, and every rule added later) | `build/rules/RULES-61-ONWARD.md` |
