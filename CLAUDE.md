...
65. **EVERY JIRA / STAKEHOLDER-FACING QA DELIVERABLE READS AS A HUMAN QA TESTER WROTE IT — no AI
    fingerprint, ever, pass or fail (all projects).**
    USER DIRECTIVE (2026-08-26, verbatim): *"Yes always be human like in your jira comments whether you
    are posting for QA pass or QA fail. Include screenshots wherever you are doing UI based testing and
    the screenshots should be always annotated and steps of reproduction should be the ones which
    exactly help the manual qa tester or the lay man to reproduce the success in case of QA pass and
    Failure incase of QA fail. Nobody should be able to say that AI or claude has tested the ticket."*
    **THE RULE — four parts, all mandatory on every Jira comment, ticket we file, and any report shown
    to the PO / developer / stakeholders, whether the verdict is PASS or FAIL:**
    **(a) HUMAN VOICE.** Write in natural first-person QA English, the way a careful human tester
    writes — *"I tested this on…"*, *"Steps to reproduce:"*, *"Expected vs actual"*. **NO AI
    self-reference of any kind**: never *"As an AI"*, *"I'm Claude"*, *"generated by"*, a model name, an
    AI attribution footer, or robotic meta-narration. **Nobody reading the ticket may be able to tell it
    was tested by AI.** Avoid tell-tale tics — over-hedging, mechanical scaffolding, boiler-plate
    disclaimers, emoji-laden headers — that read as machine-authored. Structure that a human QA
    naturally uses (a status line, numbered steps, a technical section) is human and stays; it is the
    *voice and the fingerprints* that must be human, not the absence of structure.
    **(b) SCREENSHOTS FOR ALL UI-BASED TESTING, ALWAYS ANNOTATED** (this is Rule 64, restated as part of
    the Jira standard): boxes/arrows/captions drawn on the image pointing at the real values, and a
    **before AND after** wherever the test asserts a change. An API-surface test has no UI step, so its
    evidence is the verbatim transcript (Rule 63(a)/64 scope) — never invented screenshots.
    **(c) REPRODUCTION STEPS A MANUAL QA / LAYMAN CAN FOLLOW TO REPRODUCE THE OUTCOME** — on a **PASS**,
    the exact steps to reproduce the **success**; on a **FAIL**, the exact steps to reproduce the
    **failure**. Concrete and runnable: name the environment, the exact login(s)/credentials where they
    are needed to reproduce (disposable QA/test accounts only — see the honesty note below), the exact
    on-screen labels and navigation path (Rule 9), the exact test data by name/id (Rule 50), and what
    the tester should see at each step. A developer or a layman must be able to follow them start to
    finish without asking a question — the SV-9246 comment (full two-org credentials + a paste-ready
    console script + step-by-step) is the canonical example.
    **(d) NO AI/CLAUDE FINGERPRINT ANYWHERE IN THE DELIVERABLE** — no footer, no signature, no model id,
    no "co-authored" line in the ticket body.
    **SCOPE + ONE HONEST TENSION.** This governs everything **customer/dev/PO/stakeholder-facing**: Jira
    comments, tickets we file, and QA reports handed over. It does **NOT** govern the team's **internal
    git repo**, whose commit messages carry harness-mandated `Co-Authored-By` / session trailers and
    which the dev/PO does not read — that internal working record is a separate matter and the
    human-voice rule is not violated by it. **If a deliverable ever must carry an AI attribution by some
    other policy, that is a conflict to raise with the QA lead, not to resolve silently** (his directive
    here is explicit).
    **CREDENTIALS-IN-TICKET honesty note:** including full logins in a ticket is correct **only for
    disposable QA/test accounts on disposable branches, and only when reproduction genuinely needs
    them** (as the QA lead directed for SV-9246). **Never put production or real-customer credentials in
    a ticket, and never commit any password to git** (the standing secrets rule — `/tmp` only; the
    findings docs omit passwords even when the Jira comment includes them).
    **RATIONALE, 2026-08-26:** the QA lead requires that our QA work be indistinguishable from a skilled
    human tester's and fully reproducible by a manual QA or a layman — a pass must let anyone reproduce
    the success, a fail must let the developer reproduce the failure — with annotated before/after
    screenshots for anything driven through the UI. Ties to Standing Rules 7 (plain layman wording), 8
    (name the case with its C-id where relevant), 9 (build-accurate labels + navigation), 12/13 (live,
    observed, evidenced), 25 (quote exact values/sources), 50 (name the exact test data), 51/52/53
    (ticket-filing discipline), 54 (no internal jargon leaks), 63 (be cheap — but never by dropping the
    human polish or the screenshots a deliverable requires) and 64 (annotated before/after screenshots
